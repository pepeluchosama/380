 ...
 */
public class GenerateAttendanceHoursV2 extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		if (p_From != null && p_To != null)
		{
			...
			String sqlgethours = "SELECT fecha, ad_org_id, c_bpartner_id, dia, entrada -interval '1 hour' as entrada, salida -interval '1 hour' as salida," +
					" horas_trabajadas, incidencias, compensadas, " +
					" atrasobase, " + // atraso base queda igual
					//" atrasobase::timestamp -interval '1 hour' as atrasobase, " + // atraso base con 1 hora menos
					" horasmediamannana FROM RVOFB_Reloj_Control WHERE fecha " +
					" between '"+p_From+"' AND '"+p_To+"'";
			if(p_BPartner > 0)
			...
			while(rs.next())
			{
				...
				if(entryhour.compareTo(delaybegin)>0)
					if(hoursqty > 0)//revisar
						if((entryhourdate.getTime() - delaybegin.getTime() - cff.getTime()) + actualdate.getTime() + actualdate.getTime() < actualdate.getTime())
							delaycalc = calculateHours(actualdate,0,0);
						else
							delaycalc = new Date(entryhourdate.getTime() - delaybegin.getTime() - cff.getTime() + actualdate.getTime() + actualdate.getTime());
					else
						delaycalc = new Date(entryhourdate.getTime() - delaybegin.getTime() + actualdate.getTime());
				else
					delaycalc = calculateHours(actualdate,0,0);
