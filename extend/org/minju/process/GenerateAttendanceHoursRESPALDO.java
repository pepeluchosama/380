 ...
 */
public class GenerateAttendanceHoursRESPALDO extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		if (p_From != null && p_To != null)
		{
			...
			while(rs.next())
			{
				...
				if(entryhour.compareTo(atrasocalculado)>0)
					if(hoursqty > 0)
						if((entryhourdate.getTime() - atrasocalculadodate.getTime() - cff.getTime()) < 0)
							delaycalc = calculateHours(actualdate,0,0);
						else
							delaycalc = new Date(entryhourdate.getTime() - atrasocalculadodate.getTime() - cff.getTime() + actualdate.getTime() + actualdate.getTime());
					else
						delaycalc = new Date(entryhourdate.getTime() - atrasocalculadodate.getTime() + actualdate.getTime());
				else
					delaycalc = calculateHours(actualdate,0,0);
