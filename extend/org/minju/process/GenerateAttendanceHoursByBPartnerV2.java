 ...
 */
public class GenerateAttendanceHoursByBPartnerV2 extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		if (p_From != null && p_To != null)
		{
			...
			//20181130 mfrojas se agrega campo extra, para definir si los registros a borrar estan marcados o no
			
			sql = sql + " AND IsModified like 'N' and IsModified2 like 'N'";
			...
			while(rs2.next())
			{
				...
				while(rs.next())
				{
					 ...
					 * mfrojas 
					 * 20183011
					 * Antes de crear el registro nuevo, se verificara si el registro YA existe (mismo c_bpartner_id para misma fecha)
					 * Si el registro existe con IsModified2, entonces el registro se saltara completamente. 
					 * Si el registro existe con IsModified, entonces se utilizara la hora de entrada y salida ya existente.
					 */
					
					String sqlisdefined = "SELECT coalesce(hr_attendancehours_id,0) from hr_attendancehours where " +
							" c_bpartner_id = "+rs.getInt("c_bpartner_id")+" and datetrx like '"+(java.util.Date)rs.getTimestamp("fecha")+"'";
					int attendanceid_old = DB.getSQLValue(get_TrxName(), sqlisdefined);
					
					X_HR_AttendanceHours att;
					if(attendanceid_old > 0)
						att = new X_HR_AttendanceHours(getCtx(),attendanceid_old,get_TrxName());
					else
						att = new X_HR_AttendanceHours(getCtx(),0,get_TrxName());
					
					if(att.get_ValueAsBoolean("IsModified2"))
						continue;
					...
					Date auxentry = calculateHours(actualdate,8,0); //hora minima de entrada: 7AM
					Date auxzero = calculateHours(actualdate,0,0); // hora cero
					if(!att.get_ValueAsBoolean("IsModified"))
					{
...
/*							String sqlnewexit = "SELECT coalesce(min(datetrx)::timestamp,now()) as datetrx from hr_attendanceline where (datetrx::date - " +
	...
	*/				
					...
					{
						att.setEntryHour(att.getEntryHour());
						att.setExitHour(att.getExitHour());
					}
