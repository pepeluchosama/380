...
public class ProcessDMDocument extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if(doc.getdm_document_type() != null && doc.getdm_document_type().compareTo("PB") == 0)
		{
		...
		if(P_DocAction.equals("CO") && !doc.isProcessed() && doc.getdm_document_type().compareTo("PB") != 0)
		{
			...
			doc.setDocStatus("CO");
			doc.saveEx();	
			//mfrojas obtener tipo de documento base
			String sqldocbase = "SELECT docbasetype from C_DocType where C_DocType_ID = ?";
			String docbase = DB.getSQLValueString(get_TrxName(), sqldocbase, doc.getC_DocType_ID());
			if(docbase.compareTo("DMP")==0)
				DB.executeUpdate("UPDATE DM_DocumentLinePM set processed='Y' where DM_Document_ID = "+doc.get_ID(),get_TrxName());
			return "Confirmado";
		}
