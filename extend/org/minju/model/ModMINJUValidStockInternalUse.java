 ...
 */
public class ModMINJUValidStockInternalUse implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModMINJUValidStockInternalUse ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModMINJUValidStockInternalUse.class);
	/** Client			*/
		...
		if(timing == TIMING_BEFORE_COMPLETE && po.get_Table_ID() == MInventory.Table_ID)
		{
			...
			for (int i = 0; i < lines.length; i++)
			{
						...
						&& line.getQtyInternalUse().compareTo(Env.ZERO) > 0)
				{
					...
					if(line.getQtyInternalUse().compareTo(qtyOnHand) > 0)
