...
//mfrojas 20220303 se agrega ofbforward
...
public class CreateFromShipment extends CreateFrom 
{
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//mfrojas 20220303
				//Dependiendo de ofbforward, va UOM o va un concatenado de codigo prod - nombre prod.
				if(!OFBForward.ArtilecCreateShipment())
					line.add(pp);                           //  2-UOM
				else
				{
					String sqlcode = "SELECT '   ' || value || ' - ' || coalesce(UPC,name) from m_product where m_product_id = "+rs.getInt(7);
				...
				//end mfrojas
				// Add locator
				//mfrojas dependiendo de ofbforward, puede ir precio
				if(!OFBForward.ArtilecCreateShipment())
		...
		*/
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue()) {
				...
				if(!OFBForward.ArtilecCreateShipment())
				{
					pp = (KeyNamePair) miniTable.getValueAt(i, 2); // UOM
					C_UOM_ID = pp.getKey();
				}
				else
					log.config("ART uom");
				...
				//
				//mfrojas validar info desde ART
				if(OFBForward.ArtilecCreateShipment())
				{
					C_UOM_ID = DB.getSQLValue(null, "SELECT c_uom_id from m_product where m_product_id = M_Product_ID");
					M_Locator_ID = defaultLocator_ID;
				}
	...
	protected Vector<String> getOISColumnNames()
	{
	    ...
	    //mfrojas artilec shipment price
	    if(!OFBForward.ArtilecCreateShipment())
