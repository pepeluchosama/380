...
public class SubFunctionKeys extends PosSubPanel implements ActionListener
{
	...
	//public static String ID_BotonShow = "";
	...
	public void init()
	{
		...
		//m_keys = fKeys.getKeysLevel(ID_Level, p_posPanel.f_bpartner.getM_PriceList_Version_ID());
		m_keys = fKeys.getKeysLevelUnique(ID_Level, p_posPanel.f_bpartner.getM_PriceList_Version_ID(),ID_BotonHide, p_pos.getM_Warehouse_ID(), p_posPanel.f_curLine.getM_Locator_ID());
		...
		// Se limita ciclo a máximo cols * rows botones
		for (int i = 0; i < m_keys.length && i < (cols * rows); i++)
		{	
			Color keyColor = Color.lightGray;
			MPOSKey key = m_keys[i];
			
			/*if (!ID_BotonHide.contains(Integer.toString(key.getM_Product_ID())))
			{*/
			...
			for (int i = 0; i < m_keys.length; i++)
			{
				...
				if (key.getC_POSKey_ID() == C_POSKey_ID)
				{
					...
					else
					{
						...
						p_posPanel.f_curLine.setLevelNo(ID_LevelPrice);
						//end
