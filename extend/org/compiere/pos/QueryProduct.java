	...
	implements ActionListener, MouseListener, ListSelectionListener
{
	...
	protected void init()
	{
		...
		String sqlPListLevel = "";
		if(SubFunctionKeys.ID_Level== 1)
			sqlPListLevel = " IsActive='Y' AND M_Product_ID IN (SELECT M_Product_ID FROM M_ProductPrice WHERE IsActive = 'Y' " +
		 		" AND Levels = "+SubFunctionKeys.ID_LevelPrice+" AND IsVisible = 'Y'"+
		 		" AND M_PriceList_Version_ID = "+p_posPanel.f_bpartner.getM_PriceList_Version_ID()+")"+
		 		" AND M_Locator_ID = "+p_posPanel.f_curLine.getM_Locator_ID()+
		 		" AND M_PriceList_Version_ID = "+p_posPanel.f_bpartner.getM_PriceList_Version_ID();
		else if(SubFunctionKeys.ID_LevelPrice > 1)
			sqlPListLevel = " IsActive='Y' AND M_Product_ID IN (SELECT M_Product_ID FROM M_ProductPriceRef WHERE IsActive = 'Y' " +
				" AND Levels = "+SubFunctionKeys.ID_LevelPrice+" AND IsVisible = 'Y'"+
				" AND M_PriceList_Version_ID = "+p_posPanel.f_bpartner.getM_PriceList_Version_ID()+")" +
				" AND M_Locator_ID = "+p_posPanel.f_curLine.getM_Locator_ID()+
				" AND M_PriceList_Version_ID = "+p_posPanel.f_bpartner.getM_PriceList_Version_ID();
		//ininoles se agrega validacion para que solo muestre productos de la lista de precios de la NV
		s_sqlWhere = sqlPListLevel;
	...
	//ininoles nuevo metodo que recrea el form y lo muestra
	public void setVisibleWInit (boolean aFlag)
	{
		super.setVisible (aFlag);
		if (aFlag)
			f_value.requestFocus();
	...
	public void setResultsLevels ()
	{
		String sqlPListLevel = "";
		//@mfrojas se quita el locator del where, según solicitud copesa
		//if(SubFunctionKeys.ID_Level== 1)
			sqlPListLevel = "SELECT M_Product_ID , Value, Name, UPC, SKU, QtyAvailable, QtyOnHand, PriceStd " +
					" FROM RV_WarehousePriceCopesa " +
					" WHERE  IsActive='Y' " +
					" AND M_PriceList_Version_ID = "+p_posPanel.f_bpartner.getM_PriceList_Version_ID()+
					" AND M_Locator_ID = "+p_posPanel.f_curLine.getM_Locator_ID()+
					" AND Levels = "+SubFunctionKeys.ID_Level;
		/*else if(SubFunctionKeys.ID_LevelPrice > 1)
			sqlPListLevel = "SELECT DISTINCT(M_Product_ID) as M_Product_ID, Value, Name, UPC, SKU, QtyAvailable, QtyOnHand, PriceStd " +
					" FROM RV_WarehousePriceCopesa " +
					" WHERE   IsActive='Y' AND M_Product_ID IN (SELECT M_Product_ID FROM M_ProductPriceRef WHERE IsActive = 'Y' " +
					" AND Levels = "+SubFunctionKeys.ID_Level+" AND IsVisible = 'Y'"+
					" AND M_PriceList_Version_ID = "+p_posPanel.f_bpartner.getM_PriceList_Version_ID()+")"+
	//				" AND M_Locator_ID = "+p_posPanel.f_curLine.getM_Locator_ID()+
					" AND M_PriceList_Version_ID = "+p_posPanel.f_bpartner.getM_PriceList_Version_ID();*/
		//ininoles se agrega validacion para que solo muestre productos de la lista de precios de la NV		
		...
		try
		{
			pstmt = DB.prepareStatement(sqlPListLevel, null);							
		...
		{
			log.config("error"+e3+" SQL: "+sqlPListLevel);
		}
		finally
		{
			DB.close(rs, pstmt);
			rs = null; pstmt = null;sqlPListLevel = "";
		}
		enableButtons();
	}	//	setResults
