...
import org.compiere.web.GetMD5FileServlet;
...
public class SubLines extends PosSubPanel implements ActionListener
{
	...
	private static ColumnInfo[] s_layout = new ColumnInfo[] 
	{
		...
		new ColumnInfo(Msg.translate(Env.getCtx(), "Name"), "Name", String.class),
		//new ColumnInfo(Msg.translate(Env.getCtx(), "LineNetAmt"), "LineNetAmt", BigDecimal.class), (copesa_prodprice_price(m_pricelist_version_id,"+SubFunctionKeys.ID_LevelPrice+", m_product_id))
		new ColumnInfo(Msg.translate(Env.getCtx(), "LineNetAmt"), "ROUND((copesa_prodprice_price(m_pricelist_version_id,LevelNo, m_product_id))*QtyOrdered)", BigDecimal.class), 
		new ColumnInfo(Msg.translate(Env.getCtx(), "IVA"), "round((copesa_prodprice_price(m_pricelist_version_id,LevelNo, m_product_id))*QtyOrdered*0.19)", BigDecimal.class),
		//new ColumnInfo(Msg.translate(Env.getCtx(), "Valor U. IVA"), "round(PriceActual*0.19)", BigDecimal.class),
		//new ColumnInfo(Msg.translate(Env.getCtx(), "Total con IVA"), "round(LineNetAmt*1.19)", BigDecimal.class),
		new ColumnInfo(Msg.translate(Env.getCtx(), "Total con IVA"),"round(((copesa_prodprice_price(m_pricelist_version_id,LevelNo, m_product_id))*QtyOrdered)*1.19)", BigDecimal.class),
		//new ColumnInfo(Msg.translate(Env.getCtx(), "Precio de Lista"), "round(((copesa_prodprice_price(m_pricelist_version_id,"+SubFunctionKeys.ID_LevelPrice+", m_product_id))*QtyOrdered)*1.19)", BigDecimal.class),
		new ColumnInfo(Msg.translate(Env.getCtx(), "Precio de Lista"), "ROUND(pricelist*1.19)", BigDecimal.class),
		//new ColumnInfo(Msg.translate(Env.getCtx(), "Descuento"), "round(((pricelist-priceactual)*QtyOrdered)*1.19)", BigDecimal.class),
		new ColumnInfo(Msg.translate(Env.getCtx(), "Descuento"), "ROUND((pricelist*1.19-(copesa_prodprice_price(m_pricelist_version_id,LevelNo, m_product_id))*1.19)*QtyOrdered)", BigDecimal.class),
		
		new ColumnInfo(Msg.translate(Env.getCtx(), "Socio de Negocio"), "bpLineName", String.class),
		new ColumnInfo(Msg.translate(Env.getCtx(), "Dirección"), "locationName", String.class),
		//new ColumnInfo(Msg.translate(Env.getCtx(), "Total con IVA"), "round(LineNetAmt*1.19)", BigDecimal.class),
	...
	/** Where Clause						*/
	//inioles se agrega validacion para que no muestre linea con IVA
	...
	public void init()
	{
		...
		/*m_sql = m_table.prepareTable (s_layout, s_sqlFrom, 
			...
			+ " ORDER BY Line";*/
		m_sql = m_table.prepareTable (s_layout, s_sqlFrom, 
		s_sqlWhere, false, "RVOFB_order_linetax_v")
		+ " ORDER BY Line";
	...
	//	m_table.getSelectionModel().addListSelectionListener(this);
		...
		//
		//ininoles nuevo campo sumatoria de descuento
		CLabel lDiscount = new CLabel (Msg.translate(Env.getCtx(), "Descuento"));
		summary.add(lDiscount);
		f_Discount = new VNumber("Discount", false, true, false, DisplayType.Amount, "Discount");
		f_Discount.setColumns(11, 22);
		lTotal.setLabelFor(f_Discount);
		summary.add(f_Discount);
	...
	public void actionPerformed (ActionEvent e)
	{
		...
		{
			//ininoles se modifica borrar para que vuelva a nivel 1 y borre las lineas no
			//confirmadas
			/*int rows = m_table.getRowCount();
			if (rows != 0)
			{
			...
			DB.executeUpdate("DELETE FROM C_OrderLine WHERE C_Order_ID = "+order.get_ID()+
					" AND IsConfirmed = 'N' ", null);
	...
	private void setSums(MOrder order)
	{
		...
		if (order == null || noLines == 0) //red1 WORKAROUND (noLines == 0) means total and tax in order head is false.
		{
			...
			f_total.setValue(Env.ZERO);
			f_tax.setValue(Env.ZERO);
			//ininoles nuevo campo descuento
			f_Discount.setValue(Env.ZERO);
		}
		else
		{
//			order.prepareIt(); //red1 Avoid Reserving Inventory until final process and update context directly from DB.
			...
			//red1 - end -
			BigDecimal amtTLine = DB.getSQLValueBD(null, "SELECT ROUND(SUM(copesa_prodprice_price("+p_posPanel.f_bpartner.getM_PriceList_Version_ID()+", "+SubFunctionKeys.ID_LevelPrice+", m_product_id)*QtyOrdered)) FROM RVOFB_order_linetax_v WHERE C_Order_ID = ? ", retValue.get_ID());
			//f_net.setValue(retValue.getTotalLines());
			f_net.setValue(amtTLine);
			BigDecimal amtGTotal = DB.getSQLValueBD(null, "SELECT ROUND(SUM((copesa_prodprice_price("+p_posPanel.f_bpartner.getM_PriceList_Version_ID()+", "+SubFunctionKeys.ID_LevelPrice+", m_product_id)*1.19)*QtyOrdered)) FROM RVOFB_order_linetax_v WHERE C_Order_ID = ? ", retValue.get_ID());
			//f_total.setValue(retValue.getGrandTotal());
			f_total.setValue(amtGTotal);			
			//f_tax.setValue(retValue.getGrandTotal().subtract(retValue.getTotalLines()));
			f_tax.setValue(amtGTotal.subtract(amtTLine));
			//BigDecimal amtDiscount = DB.getSQLValueBD(null, "SELECT ROUND(SUM(((pricelist*1.19)-(priceactual*1.19))*QtyOrdered)) FROM RVOFB_order_linetax_v WHERE C_Order_ID = ? ", retValue.get_ID());
			BigDecimal amtDiscount = DB.getSQLValueBD(null, "SELECT ROUND(SUM(((pricelist*1.19)-(copesa_prodprice_price("+p_posPanel.f_bpartner.getM_PriceList_Version_ID()+", "+SubFunctionKeys.ID_LevelPrice+", m_product_id)*1.19))*QtyOrdered)) FROM RVOFB_order_linetax_v WHERE C_Order_ID = ? ", retValue.get_ID());
			
			if(amtDiscount != null)
				f_Discount.setValue(amtDiscount);
			else
				f_Discount.setValue(Env.ZERO);
