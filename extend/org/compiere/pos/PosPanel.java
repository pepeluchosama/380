...
*/		//
	...
	private boolean dynInit()
	{
		...
		//
		//ininoles seteamos variables por defecto
...
*/		
		...
		m_keys = fKeys.getKeysLevel(SubFunctionKeys.ID_Level, f_bpartner.getM_PriceList_Version_ID());
		...
		f_checkout.ID_lastOrder = f_curLine.getOrder().get_ID();
		//faaguilar OFB bind keys end		
		if(f_curLine.getOrderWhitOutCreate() == null)
			newOrder();
		else
			updateInfo();		
		return true;
	...
	private boolean setMPOS()
	{
		...
		if (true)	//ininoles	se coloca logica de siempre entre por problemas al cargar ad_user_id de contexto
			poss = getPOSs (0);
		else
