...
public class MTree extends MTree_Base
{
	...
	public static int getDefaultAD_Tree_ID (int AD_Client_ID, String keyColumnName)
	{
		...
		//mfrojas prueba tree
		else if(keyColumnName.equals("CC_SelectBPLine_ID"))
			TreeType = "DO";
		//mfrojas end prueba tree
		else
		{
...
//		diagPrintTree();
	...
	private void getNodeDetails ()
	{
		...
		else
		{
			...
			//mfrojas
			if(m_editable && columnNameX == "CC_SelectBPLine")
			{
				sqlNode.append(" WHERE t.IsActive='Y'");
				log.info("contexto bpartner "+Env.getContextAsInt(Env.getCtx(), "AD_Client_ID"));
				log.info("contexto bpartner "+Env.getContextAsInt(Env.getCtx(), 2000044, 2000250, "C_BPartner_ID"));
				log.info("contexto bpartner "+Env.getContextAsInt(Env.getCtx(), 2000044, 2000251, "AD_Client_ID"));
				log.info("contexto bpartner "+Env.getContextAsInt(Env.getCtx(),0, "AD_User_ID"));
				log.info("contexto bpartner "+Env.getContextAsInt(Env.getCtx(),0, "AD_User_ID"));

				//sqlNode.append(" WHERE t.IsActive='Y'  AND c_bpartner_Id = "+Env.getContext(getCtx(), 0, "C_BPartner_ID").toString()+" ");
			}
