...
public class CalloutInvoice extends CalloutEngine
{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				if (C_BPartner_ID.toString().equals(Env.getContext(ctx, WindowNo, Env.TAB_INFO, "C_BPartner_ID")))
				{
					...
					if (loc.length() > 0 && loc.compareTo("0") != 0)
	...
	//	Discount = (BigDecimal)mTab.getValue("Discount");
			...
			&& !"N".equals(Env.getContext(ctx, WindowNo, "DiscountSchema")))
		{
			...
			// solo se setea precio si es nulo
			BigDecimal priceEnteredOld = (BigDecimal)mTab.getValue("PriceEntered");
			if(priceEnteredOld == null || priceEnteredOld.compareTo(Env.ZERO)==0)
			{
				mTab.setValue("PriceActual", pp.getPriceStd());
			//	mTab.setValue("Discount", pp.getDiscount());
	...
	public String order (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if (value==null)
			return "";
		int ID_Order = (Integer)value;
		if(ID_Order > 0)
		{
			MOrder order = new MOrder(ctx, ID_Order, null);
			...
			mTab.setValue("SalesRep_ID", order.getSalesRep_ID());
