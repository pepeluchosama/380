...
public class MCash extends X_C_Cash implements DocAction
{
	...
	{
		//mfrojas comentado codigo original (según cambio en 360)
		/** 
		...
		**/
		...
		if(getDocStatus().equals("DR"))
		{
			...
			//MFROJAS Debido a la cantidad de estados creados en MINJU, se cambia la query. En vez de estados =, se buscara estados distintos
			...
			int count = DB.getSQLValue(get_TrxName(),"select count(1) from C_Cash where DocStatus NOT IN ('CO','CL','VO','RE') and C_CashBook_ID="+getC_CashBook_ID()+" and C_Cash_ID!="+getC_Cash_ID());
			if(count>0)
			{
				log.saveError("Error", "Ya existe otra caja en borrador o en proceso por favor completela antes de crear una nueva");
								m_processMsg = "Ya existe otra caja en borrador por favor completela antes de crear una nueva";
								return false;
			}
		}
   ...
   //     	oldCash=MCash.getDuplicate(getCtx(),getC_CashBook_ID(),get_TrxName(),getC_Cash_ID());
	...
	public String completeIt()
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			{
				if (line.getC_Payment_ID()<=0){ //added by OFB
