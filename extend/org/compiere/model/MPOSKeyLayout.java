...
public class MPOSKeyLayout extends X_C_POSKeyLayout
{
	...
	public MPOSKey[] getKeysLevel (int ID_Level, int ID_PriceListVersion)
	{
		...
		if(ID_Level == 1)
		{	
			sqlProduct = "(SELECT M_Product_ID FROM M_ProductPrice WHERE IsActive = 'Y' AND IsVisible = 'Y'" +
				" AND Levels = "+ID_Level+" AND M_PriceList_Version_ID = "+ID_PriceListVersion+")";
		}
		else if(ID_Level > 1)
		{	
			sqlProduct = "(SELECT M_Product_ID FROM M_ProductPriceRef WHERE IsActive = 'Y' AND IsVisible = 'Y'" +
				" AND Levels = "+ID_Level+" AND M_PriceList_Version_ID = "+ID_PriceListVersion+")";
		}
		String sql = "SELECT * FROM C_POSKey WHERE C_POSKeyLayout_ID=? AND IsActive = 'Y' " +
				" AND M_Product_ID IN "+sqlProduct+" ORDER BY SeqNo";
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement (sql, get_TrxName());
	...
	public MPOSKey[] getKeysLevelUnique (int ID_Level, int ID_PriceListVersion, String ID_HideProduct, int _warehouseid, int _locatorid)
	{
		...
		{	
			sqlProduct = "(SELECT M_Product_ID FROM M_ProductPrice WHERE IsActive = 'Y' AND IsVisible = 'Y'" +
				" AND Levels = "+ID_Level+" AND M_PriceList_Version_ID = " + ID_PriceListVersion +
				" AND bomqtyavailablecopesa(M_Product_ID," + _warehouseid +","+ _locatorid +") > 0)";
		}
		else if(ID_Level > 1)
		{	
			sqlProduct = "(SELECT M_Product_ID FROM M_ProductPriceRef WHERE IsActive = 'Y' AND IsVisible = 'Y'" +
		...
		String sql = "SELECT * FROM C_POSKey WHERE C_POSKeyLayout_ID=? AND IsActive = 'Y' " +
				" AND M_Product_ID IN "+sqlProduct;
		if(ID_HideProduct != null && ID_HideProduct.trim().length()> 3)
			sql+= " AND M_Product_ID NOT IN ("+ID_HideProduct.trim()+")";
		sql+=" ORDER BY SeqNo";
		PreparedStatement pstmt = null;
		try
		{
