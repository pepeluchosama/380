...
//		new ColumnInfo(Msg.translate(Env.getCtx(), "M_AttributeSet_ID"), "st.Name", KeyNamePair.class, "st.M_AttributeSet_ID"), // @Trifon - Not sure if this need to be shown
	...
	private static String s_sqlWhere = "s.M_Product_ID=? AND l.M_Warehouse_ID=?";// ininoles se crea variable faltante
	...
	private void dynInit(int C_BPartner_ID)
	{
		...
		//ininoles codigo original comentado para que muestre todos los lotes y no solo los con movimiento
		/*m_sql = m_table.prepareTable (s_layout, s_sqlFrom, m_M_Warehouse_ID == 0 ? s_sqlWhereWithoutWarehouse : s_sqlWhere, false, "s")
			+ " ORDER BY asi.GuaranteeDate, s.QtyOnHand";	//	oldest, smallest first*/
	...
	private void refresh()
	{
		...
		//ininoles se copia sqls de 360 para que funcione de la misma forma
		...
		try
		{
			pstmt = DB.prepareStatement(sql, null);
			pstmt.setInt(1, m_M_Product_ID);
			/*if ( !showAll.isSelected() ) { //ininoles codigo comentado
