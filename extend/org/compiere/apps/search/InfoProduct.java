...
public class InfoProduct extends Info implements ActionListener, ChangeListener
{
	...
	private void statInit()
	{
		...
		ColumnInfo[] s_layoutWarehouse = null;
		if(OFBForward.UseNewSchemaReservedInfoProd())
		{
			s_layoutWarehouse = new ColumnInfo[]{
	        		...
	        		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyAvailable"), "SUM(QtyOnHand)-bomqtyreserved(m_product_id,m_warehouse_id, 0)", Double.class, true, true, null),
	        		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyOnHand"), "sum(QtyOnHand)", Double.class),
	           		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyReserved"), "MAX(QtyReserved)", Double.class),
	...
	//        		new ColumnInfo(Msg.translate(Env.getCtx(), "DocumentNote"), "DocumentNote", String.class)};
        ...
        //ininoles se crea nueva vista con ofbforward para blumos
        if(OFBForward.InfoProductUseNewView())
        	s_sqlFrom = " M_PRODUCT_STOCK_V2 ";
        ...
        if(OFBForward.InfoProductUseNewView())
        {
        	m_sqlWarehouse = warehouseTbl.prepareTable(s_layoutWarehouse, s_sqlFrom, s_sqlWhere, false, "M_PRODUCT_STOCK_V2");
        }
		m_sqlWarehouse += " Group By M_Warehouse_ID, WarehouseName ";
		if(OFBForward.UseNewSchemaReservedInfoProd())
			m_sqlWarehouse += ", M_Product_ID ";
...
//		validacion mostrar todas las listas de precios ininoles
        ...
        else
        {
			...
			else
			{
				...
				//ininoles no se debe setear nunca el socio de negocio relacionado
	...
	protected String getSQLWhere()
	{
		...
		//  Only Stock items
		if(OFBForward.UseInfoProductOnlyStock())
		{
			if (checkOnlyStock.isSelected())
				list.add("p.isStocked = ? AND bomQtyAvailable(p.M_Product_ID,COALESCE("+ ((Integer) fWarehouse_ID.getValue()).intValue()+",0),0) > 0");
		}
		else
		{
			if (checkOnlyStock.isSelected())
	...
	protected void showHistory(int record_id)
	{
		...
		//ininoles no se muestra historial para artilec
		if(!OFBForward.NoShowInvoiceHistory())
			ih.setVisible(true);
		ih = null;
	}	//	showHistory
	...
	protected Info_Column[] getTableLayout()
	{
		...
		//ininoles se cambia orden. Se colocan primero campos de stock en vez de precios
		if (isValidVObject(fWarehouse_ID))
		{
	...
	{
		String orderClause = "";
		if(OFBForward.OrderByInfoProdQtyAvailable())
		{
			if (isValidVObject(fWarehouse_ID))
			{
	...
	private void refreshAtpTab ()
	{
		...
		else  // Update the table
		{
			...
			if(OFBForward.NoUseDetailATPBlumos())
			{
				sql = "(";
			}
