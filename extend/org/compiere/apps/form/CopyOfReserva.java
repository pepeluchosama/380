...
*/
public class CopyOfReserva
{
	public DecimalFormat format = DisplayType.getNumberFormat(DisplayType.Amount);

	/**	Logger			*/
	public static CLogger log = CLogger.getCLogger(CopyOfReserva.class);
	...
	{		
		...
		StringBuffer sql = new StringBuffer("SELECT hh.hora,hh.med_specialty_id, h.c_bpartnermed_id ,esp.name, " +
				" tr.name," +
				...
				" WHERE p.DateTrx = ? and h.MED_Specialty_ID=? and h.C_BPartnerMed_ID=?" +
				" order by hh.hora ");
		...
		try
		{
			...
			pstmt.setInt(2, (Integer)especialidad);
			...
			while (rs.next())
			{
				...
				KeyNamePair pp = new KeyNamePair(rs.getInt(9), rs.getString(1));
				line.add(pp); // -1 hora
				...
				String NameValue = DB.getSQLValueString(null, "SELECT name FROM AD_Ref_List where AD_Reference_ID = 1000110 "+
						" and value like '"+rs.getString(10)+"'");
				line.add(NameValue==null?"Desconocido":NameValue);
				line.add(rs.getString(11)==null?"Desconocido":rs.getString(11));
				//ininoles end
				//
				data.add(line);
				log.fine(" line added");
	...
	public Vector<Vector<Object>> getReservaData(IMiniTable horasTable)
	{
		...
		Vector<Vector<Object>> data = new Vector<Vector<Object>>();
		StringBuffer sql = new StringBuffer("SELECT c.MED_Appointment_id,h.hora, c.MED_AttentionType_ID, ti.type, " +
				" c.State, c.description" +
				" , p.name  as paciente ," +
				//" t.name as titular," +
				" h.Description " +
				" from MED_Appointment c" +
				" inner join MED_AttentionType ti on (c.MED_AttentionType_ID=ti.M_ID) " +
				...
				//" inner join Jes_scheduletime h on (ch.Jes_Hora_id=h.Jes_Hora_ID) " +
				//" where ch.Jes_Hora_ID IN "+where+" order by c.fechareserva");
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//ininoles nuevo orden de campos
				String tipo="";
				if(rs.getString("Estado").equals("AN"))
					tipo="Anulada";
				else if (rs.getString("Estado").equals("RC"))
					tipo="Reservada / Confirmada";
				else if (rs.getString("Estado").equals("RN"))
					tipo="Reservada / No Confirmada";
				line.add(tipo); //5- Estado
				
				line.add(rs.getString("paciente")); //2- Paciente
				
				line.add(rs.getString("tipo")); //4- Tipo Atención
	...
	public Vector<String> getReservaColumnNames()
	{
		...
		columnNames.add(Msg.getMsg(Env.getCtx(), "Titular"));
		columnNames.add(Msg.getMsg(Env.getCtx(), "Contacto Movil"));//ininoles se cambia nombre campo descripcion por contacto movil
		columnNames.add(Msg.getMsg(Env.getCtx(), "Descripcion estado"));//ininoles se agrega nuevo campo descripcion estado
		//ininoles end
		...
		for (int i = 0; i < pRows; i++)
		{
			...
			if (((Boolean)horas.getValueAt(i, 0)).booleanValue())
			{
				...
				if(cita==null)
				{
					...
					cita.setReservationDate(hora.getTimeRequested());
				...
				*/
				...
				cita.set_CustomColumn("C_BParnterMED_ID", (Integer)tratante);
				cita.save();
				//end ininoles
			}
		}
		...
		for (int i = 0; i < pRows; i++)
		{
			...
			if (((Boolean)reservas.getValueAt(i, 0)).booleanValue())
			{
				...
				}else
				{
					...
					cita.setState((String)nuevoEstadoBox);
	...
	{		
		...
		StringBuffer sql = new StringBuffer("SELECT p.datetrx,p.med_scheduleday_ID,p.dia, ref.name as dianombre," +
				...
				"INNER JOIN med_schedule h ON (p.med_schedule_ID=h.med_schedule_ID) " +
				"INNER JOIN AD_Ref_List ref ON (AD_Reference_ID=1000075 AND ref.value = p.dia) " +
	...
	{		
		...
		Vector<Vector<Object>> data = new Vector<Vector<Object>>();
		StringBuffer sql = new StringBuffer("SELECT hh.hora,hh.med_scheduletime_id,refEstado.name as estado, " +
