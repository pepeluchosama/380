...
public class InvoiceGen extends GenForm
{
	...
	private String getOrderSQL()
	{
	    StringBuffer sql = new StringBuffer(
	            "SELECT C_Order_ID, o.Name, dt.Name, ic.DocumentNo, bp.Name, DateOrdered, TotalLines,ic.Description "
	            + "FROM C_Invoice_Candidate_v ic, AD_Org o, C_BPartner bp, C_DocType dt "
	            + "WHERE ic.AD_Org_ID=o.AD_Org_ID"
	            + " AND ic.C_BPartner_ID=bp.C_BPartner_ID"
	            + " AND ic.C_DocType_ID=dt.C_DocType_ID"
	            + " AND ic.AD_Client_ID=?"
	            //ininoles se agrega validacion para no generar mas facturas de las debidas
	...
	{
		StringBuffer sql = new StringBuffer();
	    sql.append("SELECT rma.M_RMA_ID, org.Name, dt.Name, rma.DocumentNo, bp.Name, rma.Created, rma.Amt,rma.description ");
	...
	public void executeQuery(KeyNamePair docTypeKNPair, IMiniTable miniTable)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				miniTable.setValueAt(rs.getString(8), row, 4);              //  Description
