...
public class RequisitionPOCreate extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("DateRequired"))
			{
				...
				p_DocNo = (String)para[i].getParameter();
			else
	...
	private void newOrder(MRequisitionLine rLine, int C_BPartner_ID) throws Exception
	{
		...
		if(OFBForward.GenerateOCTax())
		{
			int curr = DB.getSQLValue(get_TrxName(), "SELECT C_Currency_ID from M_Requisition where M_Requisition_ID = "+rLine.getM_Requisition_ID());
			M_PriceList_ID = DB.getSQLValue(get_TrxName(), "SELECT max(M_PriceList_ID) from M_PriceList where isactive='Y' and isSOPriceList = 'N' and C_Currency_ID = "+curr);
		}
		...
		if (m_order == null)
		{
			...
			m_order.setC_DocTypeTarget_ID(2000108);
			m_order.setBPartner(m_bpartner);
			m_order.setM_PriceList_ID(M_PriceList_ID);
			m_order.setDocumentNo(p_DocNo);
			m_order.setDocAction("CO");
	...
	private void newLine(MRequisitionLine rLine) throws Exception
	{
		...
		//mfrojas se reemplaza c_bpartner_id de linea por c_bpartner_-id de cabecera
		MRequisition req = new MRequisition(getCtx(), p_M_Requisition_ID, get_TrxName());
		int C_BPartner_ID = req.get_ValueAsInt("C_BPartner_ID");
		
		if(C_BPartner_ID == 0)
			C_BPartner_ID = DB.getSQLValue(req.get_TrxName(), "SELECT max(c_bpartner_id) from c_bpartner where name like 'Ministerio%usticia%'"); 
		if (C_BPartner_ID != 0)
		{
			;
		}
		...
		if (product != null)
		{
			...
			//@mfrojas se agrega el precio actual
			m_orderLine.setPriceActual(rLine.getPriceActual());
			m_orderLine.setPriceEntered(rLine.getPriceActual());
			m_orderLine.set_CustomColumn("M_RequisitionLine_ID", rLine.get_ID());
		}
		else
		{
		...
		if(OFBForward.GenerateOCTax())
		{
			...
			{
				 IdImpuesto = DB.getSQLValue(get_TrxName(), "SELECT C_Tax_ID from C_Tax where name like 'IVA'");
				m_orderLine.setC_Tax_ID(IdImpuesto);
			}
			else
			{
				IdImpuesto = DB.getSQLValue(get_TrxName(), "SELECT C_Tax_ID from C_Tax where name like 'Exento'");
				m_orderLine.setC_Tax_ID(IdImpuesto);
			}
			/*m_orderLine.setEditable(false);
			m_orderLine.setVisible(false);
			m_orderLine.setVisible(false);*/
		}
