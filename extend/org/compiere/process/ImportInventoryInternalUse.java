 ...
 */
public class ImportInventoryInternalUse extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		try
		{
			...
			while (rs.next())
			{
				 ...
				 * 20181023 mfrojas
				 *
				 * para efectos de inventario uso interno, se comenta la creacion de linea
				 * como inventario fisico, para crearla como internal use
				 * 
				 **/
				...
				line.setQtyBook(imp.getQtyBook());
				line.setQtyInternalUse(imp.getQtyCount());
				line.setM_AttributeSetInstance_ID(M_AttributeSetInstance_ID);
				String sqlcharge = "SELECT coalesce(max(c_charge_id),0) from c_charge where lower(name) like '%consumo%' and isactive='Y'";
				int C_Charge_ID = DB.getSQLValue(get_TrxName(), sqlcharge);
				if(C_Charge_ID == 0)
					throw new IllegalArgumentException("No hay cargo disponible ");
