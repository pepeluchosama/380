...
public class Doc_AllocationHdr extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if (invoice == null)
			{
				//	Payment Only OFB se copia metodo de 380 para que no mande error
			...
			else
			{
				...
				if (as.isAccrual())
				{
					...
					{
						//se sobreescribe cuenta por la de la factura(por defecto usa bp de la linea de la asigancion)
						//cambio usado para que funcione asignacion internacional
						int idValidC = DB.getSQLValue(getTrxName(), "SELECT max(C_APB_Acct) FROM C_BP_Vendor_Acct WHERE C_BPartner_ID="+invoice.getC_BPartner_ID()+" AND C_AcctSchema_ID="+as.get_ID());
						bpAcct = MAccount.get (as.getCtx(), idValidC);
		...
		//	reset line info
		//ininoles se agrega nuevo metodo de aplicado en asignacion en moneda extranjera
		if(OFBForward.UseAlloactionHeaderNoCLPOFB())
		{				
			boolean deleteRecord = false;
			//Se revisan si es una asignacion solo en moneda extranjera
			FactLine fl = null;
			//FactLine flForRGL = null;
			MAccount bpAcct = null;		//	Liability/Receivables
			for (int i = 0; i < p_lines.length; i++)
			{	
				...
				if(line.getC_Invoice_ID() > 0 && line.getC_Payment_ID() > 0)
				{
					...
					BigDecimal ratePay = MConversionRate.getRate(line.getC_Currency_ID(),as.getC_Currency_ID(),pay.getDateAcct(),114,pay.getAD_Client_ID(), pay.getAD_Org_ID());
					BigDecimal rateInv = MConversionRate.getRate(line.getC_Currency_ID(),as.getC_Currency_ID(),inv.getDateAcct(),114,inv.getAD_Client_ID(), inv.getAD_Org_ID());
					if(pay.getC_Currency_ID() != as.getC_Currency_ID() && inv.getC_Currency_ID() != as.getC_Currency_ID())
					{
						//se borran los registros ya creados si es la primera linea
						if(!deleteRecord)
						{
							FactLine[] factlines = fact.getLines();
							for (FactLine factline : factlines)
						...
						//se crean nueva contabilidad
						if (inv.isSOTrx())
						{
							log.config("factura de venta");
							if (line.getC_Payment_ID() != 0)
							{
								if(ratePay == null)
									ratePay = Env.ONE;
								fl = fact.createLine (line, getPaymentAcct(as, line.getC_Payment_ID()),
										as.getC_Currency_ID(), line.getAmtSource().multiply(ratePay), null);
								fl.saveEx(getTrxName());
								//se actualiza moneda y amtsource 
								DB.executeUpdate("UPDATE Fact_Acct SET C_Currency_ID = "+inv.getC_Currency_ID()+
										",  AmtSourceDr="+line.getAmtSource().abs()+" WHERE Fact_Acct_ID = "+fl.get_ID(),getTrxName());
								log.config("despues de update record_id = "+fl.get_ID());
								
							}		
							//	AR Invoice Amount	CR
							//faaguilar OFB Special Docs begin
							if (inv.getDocBase().equals("ARB"))
								bpAcct = getAccount(Doc.ACCTTYPE_ARB, as);
							else if (inv.getDocBase().equals("PTK"))
							{
								if(inv.getTenderType().equals("K"))
								bpAcct = getAccount(Doc.ACCTTYPE_PTCheck, as);
								else
								bpAcct = getAccount(Doc.ACCTTYPE_PTLetra, as);
							}
							else if (inv.getDocBase().equals("FAT"))
							{
								bpAcct = getAccount(Doc.ACCTTYPE_FAT, as);
							}
							else
							...
							//faaguilar OFB  begin 
							if( line.getAmtSource().compareTo(Env.ZERO)<0)
							{
								fl = fact.createLine (line, bpAcct,
										as.getC_Currency_ID() ,  line.getAmtSource().multiply(rateInv).abs(), null);
							...
							{
								fl = fact.createLine (line, bpAcct,
										as.getC_Currency_ID(), null,  line.getAmtSource().multiply(rateInv).abs());		//	payment currency
								
								fl.saveEx(getTrxName());
								//se actualiza moneda y amtsource 
								DB.executeUpdate("UPDATE Fact_Acct SET C_Currency_ID = "+inv.getC_Currency_ID()+
										",  AmtSourceCr="+line.getAmtSource().abs()+" WHERE Fact_Acct_ID = "+fl.get_ID(),getTrxName());
								log.config("despues de update record_id = "+fl.get_ID());
							}
							//lines de diferencia
							log.config("Linea de diferencia");
							//si factura mayor a pago
							if(line.getAmtSource().abs().multiply(rateInv).compareTo(line.getAmtSource().abs().multiply(ratePay)) > 0)
							{	
								fl = fact.createLine (line, getAccount(Doc.ACCTTYPE_CurrencyDif, as),
										as.getC_Currency_ID(),line.getAmtSource().abs().multiply(ratePay).subtract(line.getAmtSource().abs().multiply(rateInv)).abs(),null);
							}
							else if(line.getAmtSource().abs().multiply(rateInv).compareTo(line.getAmtSource().abs().multiply(ratePay)) < 0)				
							{
								fl = fact.createLine (line, getAccount(Doc.ACCTTYPE_CurrencyDif, as),
										as.getC_Currency_ID(),null,line.getAmtSource().abs().multiply(rateInv).subtract(line.getAmtSource().abs().multiply(ratePay)).abs());
							}							
						}
						else
						{
							...
							bpAcct = getAccount(Doc.ACCTTYPE_V_Liability, as);
							//faaguilar OFB begin
							if (inv.getDocBase().equals("APB")) 
								bpAcct = getAccount(Doc.ACCTTYPE_APB, as); 
							
							log.config("line.getAmtSource: "+line.getAmtSource());
							
							/*if(line.getAmtSource().compareTo(Env.ZERO)<0)
								fl = fact.createLine(line, bpAcct, 
										228,null, line.getAmtSource().multiply(rateInv).abs());
							else//faaguilar OFB end
								fl = fact.createLine (line, bpAcct,
										228, line.getAmtSource().multiply(rateInv).abs(), null);		//	payment currency
							*/
							//linea de factura
							fl = fact.createLine(line, bpAcct, 
									as.getC_Currency_ID(),line.getAmtSource().multiply(rateInv).abs(), null);
							...
							/*if (fl != null && inv != null)
							{
								fl.setC_Currency_ID(inv.getC_Currency_ID());
								fl.setAmtSourceDr(line.getAmtSource().abs());
							}*/
								
							if (fl != null && inv != null)
								fl.setAD_Org_ID(inv.getAD_Org_ID());
							
							log.config("AP Invoice Amount	DR:" + fl );
							
							//	Payment/Cash	CR
							if (line.getC_Payment_ID() != 0) // Avoid usage of clearing accounts
							{
								log.config("Dentro de pago");
								log.config("line.getAmtSource: "+line.getAmtSource());
								/*if(line.getAmtSource().compareTo(Env.ZERO)<0)				
								{
									log.config("dentro if");
									fl = fact.createLine (line, getPaymentAcct(as, line.getC_Payment_ID()),
											228,null, line.getAmtSource().abs().multiply(ratePay));
									log.config("fl pago : "+fl);
								}
								else//faaguilar OFB end
									fl = fact.createLine (line, getPaymentAcct(as, line.getC_Payment_ID()),
											228,null, line.getAmtSource().abs().multiply(ratePay));*/
								fl = fact.createLine (line, getPaymentAcct(as, line.getC_Payment_ID()),
										as.getC_Currency_ID(),null, line.getAmtSource().abs().multiply(ratePay));
								fl.saveEx(getTrxName());
								//se actualiza moneda y amtsource
								DB.executeUpdate("UPDATE Fact_Acct SET C_Currency_ID = "+pay.getC_Currency_ID()+
										",  AmtSourceCr="+line.getAmtSource().abs()+" WHERE Fact_Acct_ID = "+fl.get_ID(),getTrxName());
								log.config("despues de update record_id = "+fl.get_ID());
								/*if (fl != null && inv != null)
								{
									fl.setC_Currency_ID(pay.getC_Currency_ID());
									fl.setAmtSourceCr(line.getAmtSource().abs());
								}*/
								
								if (fl != null && pay != null)
									fl.setAD_Org_ID(pay.getAD_Org_ID());								
								log.config("Payment´:" + fl );
								//lines de diferencia
								log.config("Linea de diferencia");
								//si factura mayor a pago
								if(line.getAmtSource().abs().multiply(rateInv).compareTo(line.getAmtSource().abs().multiply(ratePay)) > 0)
								{	
									fl = fact.createLine (line, getAccount(Doc.ACCTTYPE_CurrencyDif, as),
											as.getC_Currency_ID(),null,line.getAmtSource().abs().multiply(ratePay).subtract(line.getAmtSource().abs().multiply(rateInv)).abs());
								}
								else if(line.getAmtSource().abs().multiply(rateInv).compareTo(line.getAmtSource().abs().multiply(ratePay)) < 0)				
								{
									fl = fact.createLine (line, getAccount(Doc.ACCTTYPE_CurrencyDif, as),
											as.getC_Currency_ID(),line.getAmtSource().abs().multiply(rateInv).subtract(line.getAmtSource().abs().multiply(ratePay)).abs(),null);
								}
							}	
						}
					}
