 ...
 */
public class DepreAssetsDefensoria_old_BAck extends SvrProcess
{
	...
	if(DepDoc.getDepType().equalsIgnoreCase((X_A_Asset_Dep.DEPTYPE_CorrecionMonetaria )))
		sql+=" And fore.Corrected='N' And "+ID_grupo;
	...
	/*if(DepDoc.getDepType().equalsIgnoreCase("ANC"))
	{
		...
		if (id_invoice > 0)
		{
			sql = " SELECT DISTINCT(A_Asset_ID) as A_Asset_ID FROM C_InvoiceLine WHERE C_Invoice_ID = "+DepDoc.get_ValueAsInt("C_Invoice_ID")+")" ;			
		}
		else
		{
			sql = " SELECT A_Asset_ID FROM A_Asset WHERE A_Asset_ID="+DepDoc.getA_Asset_ID();
		}		
	}*/
	...
	{		
		try
		{
			...
			while (rs.next() )
			{
				...
				else if(DepDoc.getDepType().equalsIgnoreCase("ANC"))//venta activo no contabilizable DPP
				{ 
					;
				}
			}
