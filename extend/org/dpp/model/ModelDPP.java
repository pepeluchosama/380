 ...
 */
public class ModelDPP implements ModelValidator
{
	/**
	 *	Constructor.
	...
	public String docValidate (PO po, int timing)
	{
		...
		if(timing == TIMING_AFTER_COMPLETE && po.get_Table_ID()==MInvoice.Table_ID)
		{
			...
			if (inv.getC_Order_ID()>0 && inv.isSOTrx() == false)
			{	
				MOrder order = new MOrder(inv.getCtx(), inv.getC_Order_ID(), inv.get_TrxName());
				if (order.getC_DocType_ID() == 1000057)//tipo de documento activo fijo
				{
					...
					for (int i = 0; i < lines.length; i++)
					{
						...
						{
							MAsset asset = new MAsset(inv.getCtx(), line.getA_Asset_ID(), inv.get_TrxName());
							...
							if (id_utm > 0)
							{
								amt3UTMclp = MConversionRate.convert(po.getCtx(), new BigDecimal("3.0"), id_utm, inv.getC_Currency_ID(),
										inv.getDateAcct(), 0, inv.getAD_Client_ID(), inv.getAD_Org_ID());
								
								if (inv.getC_Currency_ID() != 228)
									totalAmtLineCLP = MConversionRate.convert(po.getCtx(), line.getLineTotalAmt(), inv.getC_Currency_ID(), 228,
											inv.getDateAcct(), 0, inv.getAD_Client_ID(), inv.getAD_Org_ID());
								try {
