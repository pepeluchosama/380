 ...
 */
public class ProcessNomina extends SvrProcess
{
		...
		if (p_HR_Process > 0)
		{
			...
			while(rs.next())
			{
				...
				while(rsConcept.next())
				{
					...
					BigDecimal afp_Porcent = Env.ZERO;
					if(employee.get_Value("AFP_Porcent") != null)
						afp_Porcent = (BigDecimal)employee.get_Value("AFP_Porcent");
					if(afp_Porcent == null)
				...
				//se cierra ciclo de haberes para separar de descuentos y poder calcular bonos entremedio de ambos
				//bonos de modernidad
				...
				while(rsB.next())
				{
					...
					if (mesBonus.contains(mes) && rsB.getString("IsModernization").compareTo("Y") == 0)
					{	
				...
				//ciclo de descuentos
				PreparedStatement pstmtConceptDesc = DB.prepareStatement(sqlConcept, get_TrxName());
				pstmtConceptDesc.setInt(1, pro.getHR_Payroll_ID());
				ResultSet rsConceptDesc = pstmtConceptDesc.executeQuery();			
				while(rsConceptDesc.next())
				{
					X_HR_Concept concDesc = new X_HR_Concept(getCtx(), rsConceptDesc.getInt("HR_Concept_ID"), get_TrxName());
					if(concDesc.get_ValueAsString("TypeConcept").compareTo("D1") == 0)
					{
						...
						if(concDesc.get_ValueAsString("HR_Discount1") != null && concDesc.get_ValueAsString("HR_Discount1").trim().length() > 0)
						{
							porc = DB.getSQLValueBD(get_TrxName(), "SELECT "+concDesc.get_ValueAsString("HR_Discount1")+" FROM HR_Employee " +
									" where C_Bpartner_ID = "+rs.getInt("C_Bpartner_ID"));
							if(porc == null)
								porc = Env.ZERO;
						}
						...
						bpDet.setHR_ProcessBP_ID(pBP.get_ID());
						bpDet.setAD_Org_ID(pBP.getAD_Org_ID());
						bpDet.setHR_Concept_ID(concDesc.get_ID());
						bpDet.setAmount(amtTotal);
						bpDet.saveEx();
					}
					//bonos
					else if(concDesc.get_ValueAsString("TypeConcept").compareTo("B1") == 0)
					{
						String mesBono = concDesc.get_ValueAsString("months");
						mesBono = "-"+mesBono+"-";
						...
						if(mesBono.contains(mes))//corresponde bono
						{
							...
							//BigDecimal amtTotal = Env.ZERO;
							BigDecimal porc = Env.ZERO;
							if(concDesc.get_ValueAsString("HR_Bono1") != null && concDesc.get_ValueAsString("HR_Bono1").trim().length() > 0)
							{
								porc = DB.getSQLValueBD(get_TrxName(), "SELECT "+concDesc.get_ValueAsString("HR_Bono1")+" FROM HR_Employee " +
										" where C_Bpartner_ID = "+rs.getInt("C_Bpartner_ID"));
							}
							if(porc == null)
								porc = Env.ZERO;
				...
				//calculo de nuevo sistema de bonos no modernidad
				PreparedStatement pstmtBo = DB.prepareStatement(sqlBonus, get_TrxName());
				ResultSet rsBo = pstmtBo.executeQuery();
				BigDecimal amtRangeBo = Env.ZERO; 
				BigDecimal amtBonusBo = Env.ZERO;
				while(rsBo.next())
				{
					//se verifica si el mes corresponde
					String mesBonus = "";
					if(rsBo.getString("months") != null && rsBo.getString("months").trim().length() > 0)
						mesBonus = "-"+rsBo.getString("months")+"-";
					//ininoles se busca que exista sueldo para mes que se necesita
					flagBonos = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1)" +
							...
							" WHERE pd.C_BPartner_ID="+pBP.getC_BPartner_ID()+" AND p.C_Period_ID="+rsBo.getInt("C_Period_ID")+
							" AND p.IsActive = 'Y' AND pd.IsActive = 'Y'");				
					
					if(mesBonus.contains(mes) && flagBonos > 0 && rsBo.getString("IsModernization").compareTo("Y") != 0)
					{
						if(rsBo.getString("Type").compareTo("R") == 0) // tipo por rango
						{
							//seteo de campo a buscar por rango
							if(rsBo.getString("TypeAmount").compareTo("I") == 0) // usa monto imponible
							{
								amtRangeBo = DB.getSQLValueBD(get_TrxName(), "SELECT TotalAmt" +
										" FROM HR_ProcessBP pd" +
										" INNER JOIN HR_Process p ON (pd.HR_Process_ID = p.HR_Process_ID)" +
										" WHERE pd.C_BPartner_ID="+pBP.getC_BPartner_ID()+" AND p.C_Period_ID="+rsBo.getInt("C_Period_ID")+
										" AND p.IsActive = 'Y' AND pd.IsActive = 'Y'");									
							}
							else if(rsBo.getString("TypeAmount").compareTo("L") == 0) // usa monto liquido
							{
								amtRangeBo = DB.getSQLValueBD(get_TrxName(), "SELECT Total2" +
										" FROM HR_ProcessBP pd" +
										" INNER JOIN HR_Process p ON (pd.HR_Process_ID = p.HR_Process_ID)" +
										" WHERE pd.C_BPartner_ID="+pBP.getC_BPartner_ID()+" AND p.C_Period_ID="+rsBo.getInt("C_Period_ID")+
										" AND p.IsActive = 'Y' AND pd.IsActive = 'Y'");		 
							}
							else if(rsBo.getString("TypeAmount").compareTo("T") == 0) // usa monto tributable
							{
								amtRangeBo = DB.getSQLValueBD(get_TrxName(), "SELECT Total1" +
										" FROM HR_ProcessBP pd" +
										" INNER JOIN HR_Process p ON (pd.HR_Process_ID = p.HR_Process_ID)" +
										" WHERE pd.C_BPartner_ID="+pBP.getC_BPartner_ID()+" AND p.C_Period_ID="+rsBo.getInt("C_Period_ID")+
										" AND p.IsActive = 'Y' AND pd.IsActive = 'Y'"); 
							...
							//se busca valor
							amtBonusBo = DB.getSQLValueBD(get_TrxName(), "SELECT MAX(Amount) FROM HR_EmployeeBonusLine " +
									" WHERE IsActive = 'Y' AND HR_EmployeeBonus_ID="+rsBo.getInt("HR_EmployeeBonus_ID")+
									" AND MinAmt <= ? AND MaxAmt >= ?",amtRangeBo,amtRangeBo);
							if(amtBonusBo != null)
							{
								if(rsBo.getInt("HR_Concept_ID") > 0)
								{	
									//se crea registro de detalle BP
									X_HR_ProcessBPDetail bpDet = new X_HR_ProcessBPDetail(getCtx(), 0, get_TrxName());
									bpDet.setHR_ProcessBP_ID(pBP.get_ID());
									bpDet.setAD_Org_ID(pBP.getAD_Org_ID());
									bpDet.setHR_Concept_ID(rsBo.getInt("HR_Concept_ID"));
									bpDet.setAmount(amtBonusBo);
									bpDet.saveEx();
								}
							}
						}
						if(rsBo.getString("Type").compareTo("C") == 0) // tipo por rango por cargas
						{
							//seteo de campo a buscar por rango
							if(rsBo.getString("TypeAmount").compareTo("I") == 0) // usa monto imponible
								amtRangeBo = (BigDecimal)pBP.get_Value("TotalAmt");
							else if(rsBo.getString("TypeAmount").compareTo("L") == 0) // usa monto liquido
								amtRangeBo = (BigDecimal)pBP.get_Value("Total2");
							else if(rsBo.getString("TypeAmount").compareTo("T") == 0) // usa monto tributable
								amtRangeBo = (BigDecimal)pBP.get_Value("Total1");
							//se busca valor
							amtBonusBo = DB.getSQLValueBD(get_TrxName(), "SELECT MAX(Amount) FROM HR_EmployeeBonusLine " +
									" WHERE IsActive = 'Y' AND HR_EmployeeBonus_ID="+rsBo.getInt("HR_EmployeeBonus_ID")+
									" AND MinAmt <= ? AND MaxAmt >= ?",amtRangeBo,amtRangeBo);
							if(amtBonusBo != null)
							{
								...
								{	
									...
									bpDet.setAD_Org_ID(pBP.getAD_Org_ID());
									bpDet.setHR_Concept_ID(rsBo.getInt("HR_Concept_ID"));
									bpDet.setAmount(amtBonusBo.multiply((BigDecimal)employee.get_Value("QtyEntered")));
									bpDet.saveEx();
								}
							}
						}
