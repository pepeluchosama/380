 ...
 */
public class ProcessNominaReassessment extends SvrProcess
{
			...
			while(rs.next())
			{
				...
				while(rsConcept.next())
				{
					...
					else if(conc.get_ValueAsString("TypeConcept").compareTo("P") == 0)
					{
						...
						String sqlAttributet = "SELECT HR_ConceptRef_ID, MinValue FROM HR_Attribute WHERE HR_Concept_ID = "+conc.get_ID()+conc.get_ID()+" AND ? BETWEEN dateStart AND dateEnd ";
						if(conc.get_ValueAsString("HR_Reference1") != null && conc.get_ValueAsString("HR_Reference1").trim().length() > 0)
						{
