 ...
 *  @author mfrojas
 */
public class ModJUNAEBValidateUpdateAsset implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModJUNAEBValidateUpdateAsset ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModJUNAEBValidateUpdateAsset.class);
	/** Client			*/
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		//Se validara que, para las modificaciones, dependiendo del tipo de documento, sea el 
		//activo que se seleccionará. (Reactivacion solo para bajas, el resto, para todos menos bajas)
		...
		{	
			...
			if(typemov.compareTo("MOD")==0)
			{
				int asset = invl.getA_Asset_ID();
				String sqlsituation = "SELECT assetsituation FROM a_Asset where a_Asset_id = ?";
				String assetsituation = DB.getSQLValueString(po.get_TrxName(), sqlsituation, asset);
				
				if(doctype_id == 1000626 && assetsituation.compareTo("BAJ") != 0)
					return "Las reactivaciones son solo para bienes de baja";
				else if(doctype_id != 1000626 && assetsituation.compareTo("BAJ") == 0)
					return "No se puede realizar esta operacion para un bien de baja";
