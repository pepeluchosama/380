...
/**
 *	report GORECarteraContable
 *	
 *  @author ininoles
 *  @version $Id: GORECarteraContable.java,v 1.2 2013/12/31 00:51:02 ininoles$
 */
public class GORECarteraContable extends SvrProcess
{
	/**	The Order				*/
	...
	protected String doIt() throws AdempiereSystemError
	{
		...
		/*********/
		DB.executeUpdate("DELETE FROM T_CarteraContable", get_TrxName());
		/*********/
		...
		String sqlC = "select max(fa.ad_client_id) as ad_client_id , max(fa.ad_org_id) as ad_org_id, max(fa.isactive) as isactive, "+ 
				...
				"and fa2.account_id = fa.account_id and fa2.postingtype = fa.postingtype and fa2.c_bpartner_id = fa.c_bpartner_id "+
				...
				"inner JOIN c_bpartner cbp ON fa.c_bpartner_id = cbp.c_bpartner_id "+
				"where fa.dateacct between ? and ? "+ 
				"and ce.value between ? and ? "+
				"and fa.postingtype = 'A ' "+
				"and cbp.c_bpartner_id is not null "+
				"group by fa.account_id, fa.postingtype, fa.c_bpartner_id "+
				"order by account_id";
			...
			{				
				...
				while (rs.next())
				{
					...
					String InsertC=new String("INSERT INTO T_CarteraContable(ad_client_id,ad_org_id,isactive,created,createdby,updated,updatedby, "+
					"account_id,datetrx,dateacct,amtsourcedr,amtsourcecr,amtsourcedri,amtsourcecri,account2_id,postingtype,c_bpartner_id) "+
					...
					rs.getInt("Account_ID") +",'A',"+rs.getInt("c_bpartner_id")+")");
			...
			DB.executeUpdate("UPDATE T_CarteraContable SET AD_PInstance_ID="+p_PInstance_ID, get_TrxName());
