 ...
 *  
 */
public class ProcessAssetNetting extends SvrProcess
{
	/** Warehouse				*/
	...
	protected String doIt() throws Exception
	{
		...
		/*String sqlassets = "SELECT a_asset_id from a_depreciation_workfile where " +
				...
				" (SELECT A_Asset_ID from A_Asset where IsInPosession='Y' and isactive='Y' and AssetSituation='ALT')";
		*/
		String sqlassets = "SELECT ad.a_Asset_id from a_depreciation_workfile ad" +
				" INNER JOIN a_asset ass on (ass.a_Asset_id = ad.a_asset_id) WHERE " +
				" ROUND(ad.A_Asset_Cost) - Round(ad.a_Accumulated_depr) <= 2 AND " +
				" ROUND(ad.A_Asset_Cost) > 2 AND ass.isinposession='Y' AND " +
				" ass.isactive='Y' AND ass.AssetSituation='ALT'";
		if(p_AD_Org_ID > 0)
			sqlassets = sqlassets + " AND ass.AD_Org_ID = "+p_AD_Org_ID;
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				/*MAsset ass = new MAsset(getCtx(), wf.getA_Asset_ID() ,get_TrxName());
				ass.set_CustomColumn("AssetSituation", "TER");
				ass.saveEx();*/
			}
			rs.close ();
			pstmt.close ();
			pstmt = null;
			log.config("total de bienes "+count+". Monto total: "+amountdep+". year: "+Year);
			
			DB.executeUpdate("UPDATE a_Asset SET assetsituation='TER' WHERE assetsituation='ALT' and isactive='Y' " +
					" AND isinposession='Y' and a_asset_id in " +
					" (SELECT a_Asset_id from a_depreciation_workfile WHERE anno = "+Year+")", get_TrxName());
