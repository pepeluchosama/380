		...
		if (!isCreated) {
			...
			while (pp.next()) {
				...
				sqlins = "INSERT INTO M_ProductionLine "
						...
						+ pp.getBigDecimal("ProductionQty") + ", " //faaguilar OFB change int by BigDecimal
				...
				while (bom.next()) {
					...
					sqlins = "INSERT INTO M_ProductionLine "
							...
							+ ","
							+ (pp.getBigDecimal("ProductionQty").multiply(bom.getBigDecimal("BOMQty")) ).negate()+ "," //faaguilar OFB change formula for BigDecimal
							+ pp.getInt("M_Locator_ID") + ")";
					cnti = DB.executeUpdate(sqlins, get_TrxName());
					if (cnti == -1)
					...
					while (pl.next()) {
						...
						while (bom1.next()) {
							...
							sqlins = " INSERT INTO M_ProductionLine "
									...
									+ ","
									+ (pl.getBigDecimal("MovementQty").multiply(bom1.getBigDecimal("BOMQty"))) + ","  //faaguilar OFB change formula for BigDecimal
									+ pp.getInt("M_Locator_ID") + ")";
							cnti = DB.executeUpdate(sqlins, get_TrxName());
							if (cnti == -1)
								raiseError(
			...
			while (pl.next()) {
				...
				if (cntu == 0) {
					sqlins = "INSERT INTO M_Storage "
							...
							+ pl.getInt("AD_Org_ID")
							+ ", 'Y', SysDate, 0, SysDate, 0, "
							+ pl.getBigDecimal("MovementQty") + ", 0, 0)";
					cnti = DB.executeUpdate(sqlins, get_TrxName());
					if (cnti == -1)
				...
				pstmt.setBigDecimal(9, pl.getBigDecimal("MovementQty")); //faaguilar OFB BigDecimal
