 ...
 */
public class ExportDTEInvoiceFOLArtilecFull extends SvrProcess
{	
	/** Properties						*/
	...
	protected String doIt () throws Exception
	{
		...
		String mysql = "SELECT C_Invoice_ID FROM C_Invoice ci INNER JOIN C_Doctype dt ON (ci.C_Doctype_ID = dt.C_Doctype_ID)" +
				" WHERE dt.createxml = 'Y' AND ci.docstatus IN ('CO') AND urlxml IS NULL AND descriptionfel IS NULL";
		if(p_C_Invoice_ID > 0)
			mysql = mysql + " AND ci.C_Invoice_ID = "+p_C_Invoice_ID;
		String msg = "";
		try
		{
			...
			while(rs.next())
			{
				...
				if(inv.getC_DocType().getName().toLowerCase().contains("boleta"))
					msg= msg+" - "+CreateXMLCGBoleta(inv);
				else
					msg= msg+" - "+CreateXMLCG(inv);
			}		
			rs.close();
			pstmt.close();
			pstmt = null;
        ...
        try
        {
          ...
          //ininoles nueva referencia de guia de despacho
            ...
            mylog = "archivo";
	          ...
	          String ExportDir = company.get_ValueAsString("ExportDir");
	          try
	          {
	        	  File theDir = new File(ExportDir);
	        	  if (!theDir.exists())
	        	  {
	        		  ExportDir = company.get_ValueAsString("ExportDir2");
	        	  }
	          }
	          catch(Exception e)
	          {
