...
public class CreateAssetForecastOFB extends SvrProcess{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if(workfile==null || workfile.getA_Period_Posted()==acct.getA_Period_End())
					continue;
				log.config(" parameter last date "+p_lastdate);
				log.config(" variable last date "+lastdate);
				/*if (p_lastdate == null)
				...
				*/
				if (p_lastdate == null)
					lastdate = DB.getSQLValueTS(get_TrxName(), "select max(datedoc) from a_asset_forecast where processed='Y' and A_Asset_ID="+rs.getInt(1));
				else
					lastdate = p_lastdate;
				log.config(" variable last date after "+lastdate);
				
				replanningForecast(asset, workfile.getA_Asset_Cost().subtract(workfile.getA_Accumulated_Depr()),acct, 
						workfile.getA_Period_Posted(),lastdate);
				count++;
				lastdate = null;
