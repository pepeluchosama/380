 ...
 */
public class GOREAnalisisCuenta extends SvrProcess
{
	/**	The Order				*/
	...
	protected String doIt() throws AdempiereSystemError
	{
		...
		/*********/
		DB.executeUpdate("DELETE FROM T_AnalisisCuenta", get_TrxName());
		/*********/
		...
		String sqlC = "SELECT ad_client_id,ad_org_id,isactive,created,createdby,updated,updatedby, "+
				...
				"FROM fact_acct WHERE dateacct between ? and ? and Account_ID = ?";
				
		String sqlCPT = "";
		
		if (p_postingtype.equals("A"))
		{
			sqlCPT = " and postingtype like 'A'";
		}
		else if (p_postingtype.equals("B"))
		{
			sqlCPT = " and postingtype like 'B'";
		}
		else if (p_postingtype.equals("R"))
		{
			sqlCPT = " and postingtype like 'R'";
		}
		else 
		{
			sqlCPT = "";
		...
		sqlC = sqlC + sqlCPT;
			...
			{				
				...
				while (rs.next())
				{
					...
					String Insert=new String("INSERT INTO T_AnalisisCuenta(ad_client_id,ad_org_id,isactive,created,createdby,updated,updatedby,"+
					"account_id,datetrx,dateacct,c_period_id,postingtype,c_currency_id,"+
					"amtsourcedr,amtsourcecr,amtacctdr,amtacctcr,c_bpartner_id,ad_orgtrx_id,c_project_id,description) "+
			...
			DB.executeUpdate("UPDATE T_AnalisisCuenta SET AD_PInstance_ID="+p_PInstance_ID, get_TrxName());
