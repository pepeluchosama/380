 ...
 */
public class ExportDTEMMovFOL2Artilec extends SvrProcess
{
	/** Properties						*/
	...
	{
		MMovement inOut=new MMovement(m_ctx,p_M_Movement_ID, get_TrxName());
	...
	public String CreateXMLCG(MMovement inOut)
    {
		String wsRespuesta = "";
        ...
        try
        {
          ...
          //nuevo campo ciudad
            ...
            //tag contacto ma da error en el portal
            /*String strContacto = bloc.get_ValueAsString("Phone")+" - "+bloc.get_ValueAsString("Phone2");       
            dirRecepStr = dirRecepStr.replace("'", "");
            dirRecepStr = dirRecepStr.replace("\"", "");		
            
            Element ContactoRecep = document.createElement("Contacto");
            org.w3c.dom.Text contacto = document.createTextNode(strContacto); //nombre completo contacto
            ...
            MMovementLine iLines[] = inOut.getLines(true);
            //se calculan totales
            for(int i = 0; i < iLines.length; i++)
            {
            	...
            	if(iLine.getM_Product_ID() > 0)
            	{
	            	priceT = DB.getSQLValueBD(inOut.get_TrxName(), "SELECT PriceStd FROM M_ProductPrice WHERE M_PriceList_Version_ID=2000004 AND M_Product_ID="+iLine.getM_Product_ID());
	            	if(priceT == null)
            ...
            for(int i = 0; i < iLines.length; i++)
            {
                ...
                org.w3c.dom.Text UItxt = document.createTextNode(iLine.getM_Product().getC_UOM().getUOMSymbol().trim().compareToIgnoreCase("Ea")==0?"UN":iLine.getM_Product().getC_UOM().getUOMSymbol());
            ...
            if(inOut.getPOReference() != null && inOut.getPOReference().length() > 0)//referencia orden
            {
                 ...
                 fechareferencia = inOut.getDateReceived().toString().substring(0, 10);
            	 tipo_Ref = 2; //Orden
            }
            ...
            {            
	            ...
	            org.w3c.dom.Text a6Text = document.createTextNode("No constituye venta solo traslado - "+inOut.getDescription());
