 ...
 */
public class ExportDTEInvoiceFOLTestTC extends SvrProcess
{	
	/** Properties						*/
    ...
    {	
        ...
        try
        {
            ...
            mylog = "detalle";
            //se trabajaran agrupaciones
            //por defecto se agrupara por tipo producto y Nro local
            String MySqlCG = "SELECT SUM(cil.QTYEntered) as QTYEntered, SUM(cil.LineNetAmt) as LineNetAmt, " +
            		" AVG(cil.PriceEntered) as PriceEntered, SUM(cil.LineTotalAmt) as LineTotalAmt," +
            		" 'Tramo:'||MAX(tr.name) as desc, MAX(cil.M_Product_ID) as M_Product_ID, MAX(cil.C_Charge_ID) as C_Charge_ID" +
            		" FROM C_InvoiceLine cil" +
            		" LEFT JOIN M_inOutLine ml ON (cil.M_inOutLine_ID = ml.M_inOutLine_ID)" +
            		" LEFT JOIN M_product mp ON (cil.M_product_ID= mp.M_product_ID)" +
            		" LEFT JOIN C_Bpartner_Location tr ON (ml.C_Bpartner_Location3_ID = tr.C_Bpartner_Location_ID)" +
            		" WHERE C_Invoice_ID = ? " +
            		" GROUP BY ml.C_Bpartner_Location3_ID, mp.M_product_Category_ID";
            if(BP.get_ValueAsString("InvoiceTypeXML").compareTo("CLO") == 0)
            {
            	MySqlCG = "SELECT SUM(cil.QTYEntered) as QTYEntered, SUM(cil.LineNetAmt) as LineNetAmt, " +
            		" AVG(cil.PriceEntered) as PriceEntered, SUM(cil.LineTotalAmt) as LineTotalAmt," +
            		" 'Local Nro:'||MAX(tr.name) as desc, MAX(cil.M_Product_ID) as M_Product_ID, MAX(cil.C_Charge_ID) as C_Charge_ID" +
            		" FROM C_InvoiceLine cil" +
            		" LEFT JOIN M_inOutLine ml ON (cil.M_inOutLine_ID = ml.M_inOutLine_ID)" +
            		" LEFT JOIN M_InOut mio ON (ml.M_InOut_ID = mio.M_InOut_ID)" +
            		" LEFT JOIN M_product mp ON (cil.M_product_ID= mp.M_product_ID)" +
            		" LEFT JOIN C_Bpartner_Location tr ON (mio.C_Bpartner_Location_ID = tr.C_Bpartner_Location_ID)" +
            		" WHERE cil.C_Invoice_ID = ?" +
            		" GROUP BY mio.C_Bpartner_Location_ID, mp.M_product_Category_ID";
            }
            else if(BP.get_ValueAsString("InvoiceTypeXML").compareTo("CTO") == 0)
            {
            	MySqlCG = "SELECT SUM(cil.QTYEntered) as QTYEntered, SUM(cil.LineNetAmt) as LineNetAmt, " +
            		" AVG(cil.PriceEntered) as PriceEntered, SUM(cil.LineTotalAmt) as LineTotalAmt," +	        		
	        		" 'OC:'||MAX(co.poreference) as desc, MAX(cil.M_Product_ID) as M_Product_ID, MAX(cil.C_Charge_ID) as C_Charge_ID" +
	        		" FROM C_InvoiceLine cil" +
	        		" LEFT JOIN M_inOutLine ml ON (cil.M_inOutLine_ID = ml.M_inOutLine_ID) " +
	        		" LEFT JOIN M_product mp ON (cil.M_product_ID= mp.M_product_ID)" +
	        		...
	        		" WHERE cil.C_Invoice_ID =?" +
	        		" GROUP BY co.C_Order_ID, mp.M_product_Category_ID";
            }
    		...
    		try
    		{
    			...
    			while (rs.next())
    			{
	                ...
	                //en la descripcion va la informacion adicional
	                if (rs.getString("desc") != null && rs.getString("desc") != ""
	                	&& rs.getString("desc").trim().length() > 0)
	                {
		                Element DscItem = document.createElement("DscItem");
		                org.w3c.dom.Text desc = document.createTextNode(rs.getString("desc").trim());
		                DscItem.appendChild(desc);
		                Detalle.appendChild(DscItem);
	                }
	                
	                //logica cantidad
	                String qtD = Integer.toString(rs.getInt("QTYEntered"));
	                ...
	                //logica precio
	                String prD = rs.getBigDecimal("PriceEntered").setScale(0, 4).toString();
	                Element PrcItem = document.createElement("PrcItem");
	                org.w3c.dom.Text pa = document.createTextNode(prD);
	                PrcItem.appendChild(pa);
	                Detalle.appendChild(PrcItem);
	                //logica neto
	                String netD = rs.getBigDecimal("LineNetAmt").setScale(0, 4).toString();
	                Element MontoItem = document.createElement("MontoItem");
	                org.w3c.dom.Text tl = document.createTextNode(netD);
