 ...
 *  @version $Id: DepreAssetsOFBMenu.java,v 1.3 2006/02/24 07:15:43 jjanke Exp $
 */
public class DepreAssetsOFBMenu extends SvrProcess
{

	/** Properties						*/
	private Properties 		m_ctx;
	/*Order ID*/
	private int 			AssetDep_ID;
	...
	protected void prepare()
	{
		...
		//	log.fine("prepare - " + para[i]);
			if (para[i].getParameter() == null)
				;
			else if (name.equals("A_Asset_Dep_ID")){
				AssetDep_ID = para[i].getParameterAsInt();
			}
			else
	...
	{
		DepDoc= new X_A_Asset_Dep(Env.getCtx(),AssetDep_ID,get_TrxName());
		...
		MOrg orgP = new MOrg(DepDoc.getCtx(), DepDoc.getAD_Org_ID(), get_TrxName());
		...
		{		
			try
			{
				...
				while (rs.next() )
				{
					...
					{
						MInvoice inv = new MInvoice(DepDoc.getCtx(), DepDoc.get_ValueAsInt("C_Invoice_ID"), get_TrxName());
					...
					count++;
					MAsset assetTemp = new MAsset(DepDoc.getCtx(), rs.getInt("a_asset_id"), get_TrxName());
					...
					if(!DepDoc.getDepType().equalsIgnoreCase("VCP") && !DepDoc.getDepType().equalsIgnoreCase("DDT") && !DepDoc.getDepType().equalsIgnoreCase("RRV"))//baja para activos sin planificacion
			 ...
			 for (int i = 0; i < journals.length; i++)
				{
					...
					X_A_Asset asset = new X_A_Asset (DepDoc.getCtx(), jour.get_ValueAsInt("A_Asset_ID"), null);
					...
					if(DepDoc.getDepType().equalsIgnoreCase((X_A_Asset_Dep.DEPTYPE_Depreciacion ))){
						...
						X_A_Asset_Forecast fore=new X_A_Asset_Forecast (DepDoc.getCtx(),jour.get_ValueAsInt("A_Asset_Forecast_ID"),null);
						...
						MDepreciationWorkfile workfile=MAsset.getWorkFile(jour.get_ValueAsInt("A_Asset_ID"));
						MAssetAcct group=new MAssetAcct(DepDoc.getCtx(), MAssetAcct.getAssetAcct_ID(jour.get_ValueAsInt("A_Asset_ID")),get_TrxName());
			...
			if(DepDoc.getDepType().equalsIgnoreCase("ANC"))
			{
				...
				while (rsANC.next())
				{
					if (rsANC.getInt("A_Asset_ID") > 0)
						{
						X_A_Asset assetANC = new X_A_Asset (DepDoc.getCtx(), rsANC.getInt("A_Asset_ID"), null);
			 ...
			 for (int i = 0; i < journals.length; i++)
				{
					...
					if(DepDoc.getDepType().equalsIgnoreCase((X_A_Asset_Dep.DEPTYPE_Depreciacion ))){
						...
						X_A_Asset_Forecast fore=new X_A_Asset_Forecast (DepDoc.getCtx(),jour.get_ValueAsInt("A_Asset_Forecast_ID"),get_TrxName());
						...
						MDepreciationWorkfile workfile=MAsset.getWorkFile(jour.get_ValueAsInt("A_Asset_ID"));
						//MAsset asset=new MAsset(Env.DepDoc.getCtx(),jour.get_ValueAsInt("A_Asset_ID"),get_TrxName());
						MAssetAcct group=new MAssetAcct(DepDoc.getCtx(), MAssetAcct.getAssetAcct_ID(jour.get_ValueAsInt("A_Asset_ID")),get_TrxName());
						BigDecimal acum=DB.getSQLValueBD(get_TrxName(),  "select sum(AmtAcctDr) from gl_journalline where gl_journal_id="+jour.getGL_Journal_ID()
								+" and C_ValidCombination_ID="+group.getA_Depreciation_Acct());
