 ...
 */
public class ExportDTEInvoiceCGBaskakow extends SvrProcess
{
	/** Properties						*/
	...
	public String CreateXMLCG(MInvoice invoice)
    {
        ...
        try
        {
            ...
            if (pterm.getDescription() != null && pterm.getDescription().length()>0)            	
            {
	        ...
	        */
            ...
            //inioles nuevos campos pedidos por karina
            Element CiudadRecep2 = document.createElement("CiudadRecep");
            org.w3c.dom.Text cui = document.createTextNode(bloc.getLocation(true).getAddress2() == null?"Santiago":bloc.getLocation(true).getAddress2());
            CiudadRecep2.appendChild(cui);
            Receptor.appendChild(CiudadRecep2);
            
            Element DirPostal = document.createElement("DirPostal");
	        org.w3c.dom.Text dirPostalStr = document.createTextNode(obra.getName());
	        DirPostal.appendChild(dirPostalStr);
	        Receptor.appendChild(DirPostal);
	        //ininoles end
    		...
    		try
    		{
    			...
    			while (rs.next())
    			{
    				...
    				if(rs.getInt("M_PRODUCT_ID")==0 && rs.getInt("C_CHARGE_ID")==0 && 
    						rs.getInt("M_Product_Name_ID")==0)    				
                		continue;
    				//solo toma lineas positivas
                	if (rs.getBigDecimal("LINENETAMT").compareTo(Env.ZERO)>=0)
                	{    				
          ...
          //se agrega nombre de obra ininoles
	        ...
	        //se agrega referencia de guias de despacho
            //se cambia por ciclo de referencias de cada guia
            String sqlGuias = "select distinct(mio.m_inout_id) as m_inout_id " +
    		...
    		try
    		{
    			...
    			while (rs2.next())
    			{
    				...
    				if ( rs2.getInt("m_inout_id") > 0)
    				{
	                    ...
	                    org.w3c.dom.Text fchrefIO = document.createTextNode(fechaRefIO);
