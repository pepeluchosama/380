 ...
 */
public class ImportOrderWS extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		int no = 0;
		m_AD_Client_ID = 2000006;
		m_AD_Org_ID = 2000305;
		m_deleteOldImported = true;
		m_docAction = "PR";
		...
		//Agregar almacen ACIES
		sql = new StringBuffer ("UPDATE I_Order o "
				  + "SET M_Warehouse_ID=(SELECT M_Warehouse_ID FROM M_Warehouse w"
				  + " WHERE lower(description) like '%acies%') "
				  + "WHERE M_Warehouse_ID IS NULL"
				  + " AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no != 0)
			log.fine("Set Warehouse ACIES=" + no);
		...
		try
		{
			...
			while (rs.next ())
			{
					...
					|| !oldDocumentNo.equals(cmpDocumentNo))
				{
					...
					//mfrojas validate documentno
					if(imp.isSOTrx())
					{
						String sqlvalidatedoc = "SELECT count(1) from c_order where " +
								" issotrx='Y' and documentno like '"+imp.getDocumentNo()+"' and " +
								" c_doctypetarget_id = "+imp.getC_DocType_ID();
						log.config("sqlvalidatedoc "+sqlvalidatedoc);
						int validatedoc = DB.getSQLValue(get_TrxName(), sqlvalidatedoc);
						if(validatedoc > 0)
						{
							sql = new StringBuffer ("UPDATE I_Order "
									  + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=DocumentNo repeated, ' "
									  + "WHERE i_order_id = "+imp.get_ID()+"");
								no = DB.executeUpdate(sql.toString(), get_TrxName());
						}
					}
					order.saveEx();
