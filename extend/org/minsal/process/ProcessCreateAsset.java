	...
	protected String doIt() throws Exception
	{
		...
		{	
			...
			if(inv.getDocStatus().compareTo("DR") == 0 && dType.get_ValueAsString("Type1").compareTo("ALT") == 0)
			{
				...
				//mfrojas 20190403
				//Validacion: no puede haber altas con el mismo proveedor y numero de documento respaldo contable
				
				int idactual = 0;
				idactual = inv.get_ID();
				
				int bpartneractual = inv.getC_BPartner_ID();
				String docnumberinvoice = inv.get_Value("DocNumberInvoice").toString();
				
				String sqlvalidate = "SELECT count(1) from c_invoice where c_bpartner_id = "+bpartneractual+" " +
						" AND DocNumberInvoice like '"+docnumberinvoice+"' AND isactive='Y' and c_invoice_id != "+idactual;
				
				int validatesamevendordoc = DB.getSQLValue(get_TrxName(), sqlvalidate);
				if(validatesamevendordoc > 0)
					msgError = "ERROR: Este proveedor ya tiene el mismo numero de documento respaldo contable asociado";
		...
		for (int lineIndex = 0; lineIndex < lines.length; lineIndex++)
		{
			...
			if(isAsset(iLine) && iLine.isActive())
			{
				...
				{	
					...
					asset.set_CustomColumn("AD_OrgRef_ID", iLine.get_ValueAsInt("AD_OrgRef_ID"));
					...
					//mfrojas 20190830
					//Si el tipo de documento es sanitario o arriendo es no contable siempre.
					if(inv.getC_DocTypeTarget().getName().toLowerCase().contains("arriendo") || inv.getC_DocTypeTarget().getName().toLowerCase().contains("sanitar") )
						asset.setIsInPosession(false);
...
/*				if(iLine.get_ValueAsInt("S_Resource_ID")>0)
				...
				*/
				...
				if(inv.getC_DocTypeTarget_ID()==2000165)
					asset.set_CustomColumn("AssetSituation","COM");
				asset.saveEx(get_TrxName());				
			...
			if(iLine.getA_Asset_ID() > 0 && iLine.isActive())
			{
				...
				if(dType.get_ID() == 2000167)
				{
					...
					if(amtDiff.compareTo(Env.ZERO) != 0)
					{
						...
						asset.saveEx();
						DB.executeUpdate("UPDATE a_Depreciation_workfile SET a_asset_cost = "+iLine.getPriceEntered().add(amtDiff)+" where a_asset_id = "+A_Asset, get_TrxName());
				...
				else if(dType.get_ID() == 2000168)
				{
					...
					if(amtDiff.compareTo(Env.ZERO) != 0)
					{
						...
						asset.saveEx();
						DB.executeUpdate("UPDATE a_Depreciation_workfile SET a_asset_cost = "+iLine.getPriceEntered().subtract(amtDiff)+" where a_asset_id = "+A_Asset, get_TrxName());
				...
				if(dType.get_ID() == 2000167 || dType.get_ID() == 2000168)
				{
