...
/**
 *	Insertar usuario en Despacho (MINSAL)
 *  @author jleyton
 */
public class ModUpdateADUserReq implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModUpdateADUserReq ()
	{
		super ();
	}	//	MyValidator
	...
	public String docValidate (PO po, int timing)
	{
		...
		if((timing == TIMING_AFTER_COMPLETE)&& po.get_Table_ID()==MRequisition.Table_ID) {
			...
			// se valida que solo la solicitud de "Consumo" active el model
			if(req.getC_DocType().getName().contains("Consumo")) {
				log.config("Inventory = "+req.get_Value("M_Inventory_ID").toString());
				MInventory inv = new MInventory(po.getCtx(),(int) req.get_Value("M_Inventory_ID"),po.get_TrxName());
				log.config("User_ID = "+req.getAD_User_ID());
				inv.set_ValueOfColumn("AD_User_ID",req.getAD_User_ID());
				inv.save();
