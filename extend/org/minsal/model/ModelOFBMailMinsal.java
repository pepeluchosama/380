 ...
 */
public class ModelOFBMailMinsal implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModelOFBMailMinsal ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModelOFBMailMinsal.class);
	/** Client			*/
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if (type == TYPE_AFTER_CHANGE && TIDs.contains(po.get_Table_ID()) && po.is_ValueChanged("DocStatus"))
		{
			...
			{
				//se buscan correos directos configurados 
				sqlMail = "SELECT * FROM R_MailText WHERE Name like '%CORREO DIRECTO%' AND C_DocType_ID = "+po.get_ValueAsInt("C_Doctype_ID");
				try 
				{
					...
					while(rs.next())
					{
								...
								&& po.get_Value("DocStatus").toString().compareTo(rs.getString("DocStatus")) ==0)
						{
							...
							else
							{
								...
								//mfrojas 20221117
								//El primer destinatario del correo debe ser el mismo usuario del documento, en caso de que exista.

								if(po.get_Value("AD_User_ID") != null)
								{
									log.config("usuario documento "+po.get_ValueAsInt("AD_User_ID"));
									String mailcc2 = DB.getSQLValueString(po.get_TrxName(), "SELECT max(EMail) from ad_user where ad_user_id = "+po.get_ValueAsInt("AD_User_ID"));
									if(mailcc2 != null && mailcc2.trim().length()>5)
									{
										UsedTO=true;
										mailTO=mailcc2;
									}
							...
							//mfrojas 20221117
							//Si el correo tiene marcada la casilla supervisor, se debe agregar.
							log.config("supervisor "+rs.getString("IsForSupervisor"));
							if(rs.getString("IsForSupervisor").compareTo("Y") == 0)
							{
								//Se debe agregar el supervisor
								log.config("CORREO SUPERVISOR ");
								String mailcc2 = DB.getSQLValueString(po.get_TrxName(), "SELECT max(EMail) from ad_user where ad_user_Id in (SELECT supervisor_id from ad_User where ad_user_Id = ?)",po.get_ValueAsInt("AD_User_ID"));
								if(mailcc2 != null && mailcc2.trim().length()>5)
									mail.addCc(mailcc2);
							}
