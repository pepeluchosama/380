 ...
 */
public class GenOCCencosudOld extends SvrProcess
{
		    	...
		    	if(ID_Locator > 0)
		    	{
		    		...
		    		for (int y = 8; y <= 15; y++)
				    {
		    					...
		    					&& datosTV.get(x)[y].trim().compareTo("-") != 0)
		    			{
		    				...
		    				inv.set_CustomColumn("ViaAbast", datosTV.get(x)[1]);
			...
			if(p_dateTrx != null)
			{
				...
				{	
					...
					if(q5 == null)
						q5 = Env.ZERO;
					//Q4 Pronostico
					BigDecimal q4 = DB.getSQLValueBD(get_TrxName(), "SELECT SUM(Qty) FROM PP_ForecastDefinitionLine WHERE IsActive='Y' " +
					...
					BigDecimal q4Temp = DB.getSQLValueBD(get_TrxName(), "SELECT SUM(Qty) FROM PP_ForecastDefinitionLine WHERE IsActive='Y' " +
					...
					if(dayFinP2Str != null)
					{
						...
						BigDecimal q1 = DB.getSQLValueBD(get_TrxName(), "SELECT SUM(Qty) FROM PP_ForecastDefinitionLine WHERE IsActive='Y' " +
