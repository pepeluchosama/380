...
{	
	...
	{	
		...
		if(para.isINCLUIR_INDENT())
		{
			...
			t_indent = " ESINDENT<>1 AND ";
		//se agrega incluir o no solutec 10-09-2020
		String t_ver_venta_directa_solutec ="";
		if(!para.get_ValueAsBoolean("ver_venta_directa_solutec"))
			t_ver_venta_directa_solutec = "  es_venta_directa_solutec <> 'Y' AND ";
		...
		//{	
			String queryDet = "SELECT AD_CLIENT_ID,AD_ORG_ID,ISACTIVE,SYSDATE,100,SYSDATE,100,c_invoice_id, c_order_id,c_invoiceline_id," +
				...
				"VENDEDORCARTERA_ID,CLASSIFICATION,ULTIMA_FECHA_DESPACHO,PLANTA_ID,CODIGO_BLUMOS,QTYONHAND,representada_padre_id,c_projectofb_id,ESINDENT, es_venta_directa_solutec FROM "+v_la_vista+
				" WHERE "+t_representada+t_cliente+t_producto+t_vendedor+t_desde+t_hasta+t_ad_org_id+t_solutec+t_codigoBlumos+t_indent+t_ver_venta_directa_solutec+" AD_Client_ID = "+para.getAD_Client_ID();
			...
			while (rsDet.next())
			{
				...
				if(rsDet.getString("es_venta_directa_solutec").compareToIgnoreCase("Y") ==0)
					det.set_CustomColumn("es_venta_directa_solutec", "Y");
				else
					det.set_CustomColumn("es_venta_directa_solutec", "N");
			...
			if(para.isCOMPARATIVO())
			{
				...
				"FOREIGNPRICE,VENTA_MON_EXTRANJERA, DATEINVOICED, VENTA_EN_DOLAR, C_DocType_ID, SOLUTEC, COSTOUNITARIO," +
				"VENDEDORCARTERA_ID,CLASSIFICATION,ULTIMA_FECHA_DESPACHO,PLANTA_ID,CODIGO_BLUMOS,QTYONHAND,representada_padre_id,c_projectofb_id,ESINDENT,es_venta_directa_solutec FROM "+v_la_vista+
				" WHERE "+t_representada+t_cliente+t_producto+t_vendedor+t_desdeB+t_hastaB+t_ad_org_id+t_solutec+t_codigoBlumos+t_indent+t_ver_venta_directa_solutec+" AD_Client_ID = "+para.getAD_Client_ID();
				...
				while (rsDetC.next())
				{
					...
					if(rsDetC.getString("es_venta_directa_solutec").compareToIgnoreCase("Y") ==0)
						detC.set_CustomColumn("es_venta_directa_solutec", "Y");
					else
						detC.set_CustomColumn("es_venta_directa_solutec", "N");
