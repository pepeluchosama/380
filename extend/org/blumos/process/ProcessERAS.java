 ...
 */
public class ProcessERAS extends SvrProcess
{	
	/**
	...
	{	
		...
		PSchema_ID = para.get_ValueAsInt("C_AcctSchema_ID");
		//se crean variable
		...
		if(DB.isOracle())
		{
			sqlVar = "select AD_CLIENT_ID,AD_ORG_ID,ISACTIVE,SYSDATE,100,SYSDATE,100,c_elementvalue_id," +
				...
				" from fact_acct fa WHERE fa.C_AcctSchema_ID="+PSchema_ID+" AND fa.dateacct >= ? AND fa.dateacct <= ? "+sql_var_org+" GROUP BY fa.account_id" +
				" union all" +
				...
				" from fact_acct fa WHERE fa.C_AcctSchema_ID="+PSchema_ID+" AND fa.dateacct >= ? AND fa.dateacct <= ? "+sql_var_org+" GROUP BY fa.account_id) saldos on (c_elementvalue.c_elementvalue_id=saldos.account_id)" +
