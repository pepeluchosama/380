 ...
 */
public class WSTecPedidoEntradaRequisition extends SvrProcess
{	
	/** Properties						*/
	...
	{
		MRequisition req = new MRequisition(Env.getCtx(),p_M_Requisition_ID,get_TrxName());
		String msg = "";
		msg = llamarWSPedidoEntrada(req);
		return msg;
	}	//	doIt
	 
    private String llamarWSPedidoEntrada(MRequisition req) throws Exception
    {
    	String DateStr = req.getDateDoc().getDate()+"/"+(req.getDateDoc().getMonth()+1)+"/"+
    			(req.getDateDoc().getYear()+1900)+" "+req.getDateDoc().getHours()+":"+req.getDateDoc().getMinutes();
    	String mensaje = "";	
		...
		for (MRequisitionLine line : lines)
		{
			if (line.getM_Product_ID()>0)
			{
				request = request+"<DetalleEntrada>"+
			              ...
			              "<cantidad>"+line.getQty().intValue()+"</cantidad>"+
