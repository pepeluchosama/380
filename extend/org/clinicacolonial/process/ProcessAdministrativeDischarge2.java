...
public class ProcessAdministrativeDischarge2 extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		if (p_AdmDis_ID > 0)
		{
			...
			//mfrojas 20220930
			//Validacion inicial. Fecha ingreso (en cchosp) no puede ser posterior a egreso (en cc_administrativedischarge)
			
			if(adm.getDate1().compareTo(adm.getCC_Hospitalization().getDateInvited()) < 0)
				return "No se puede dar de alta por problema de fechas.";
			if(adm.isProcessed() == false && p_Action.compareTo("AL") == 0)
			{
				...
				// se desactiva hospitalization para que no se sigan agregando cargos y consumos 15-03-2022
				DB.executeUpdate("Update cc_hospitalization set IsReadOnly1='Y', IsActive='N' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(), get_TrxName());
