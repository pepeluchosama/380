...
public class ProcessAdministrativeDischarge3 extends SvrProcess
{
		...
		if (p_AdmDis_ID > 0)
		{
			...
			if(adm.isProcessed() == false && p_Action.compareTo("AL") == 0)
			{
				...
				*/
				...
				DB.executeUpdate("UPDATE CC_Diagnostics set processed='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
				DB.executeUpdate("UPDATE CC_Benefits set processed1='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
				DB.executeUpdate("UPDATE CC_Internal_Hosp_Kine set processed='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
				DB.executeUpdate("UPDATE CC_OccupationalTherapy set processed='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
				DB.executeUpdate("UPDATE CC_SpeechTherapist set processed='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
