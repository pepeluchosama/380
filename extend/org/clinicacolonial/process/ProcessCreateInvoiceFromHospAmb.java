...
public class ProcessCreateInvoiceFromHospAmb extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (p_Hospitalization_ID > 0)
		{
			...
			//nuevo metodo seteará todos los detalles solo ininoles 12-01-2022
			//se intenta crear boleta afecta
			int cantAfecta = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1)"
					+ " FROM med_labdetail m "
					+ " INNER JOIN m_product mp ON (m.M_product_ID = mp.m_product_id)"
					+ " WHERE cc_hospitalization_id = "+p_Hospitalization_ID
					+ " AND mp.IsTaxed = 'Y'"
					+ " AND m.MED_PhysicalBond <> 'Y'"
					+ " AND InsurerLine <= 0");
			if(cantAfecta > 0)
			//se crea boleta afecta				
			{
				MInvoice inv = new MInvoice(getCtx(), 0, get_TrxName());
				inv.setAD_Org_ID(hosp.getAD_Org_ID());
				inv.setC_DocTypeTarget_ID(2000171);
				inv.setC_BPartner_ID(hosp.getC_BPartner_ID());
				inv.setDateInvoiced(hosp.getUpdated());
				inv.setDateAcct(hosp.getUpdated());
				//se copia campo desde ingreso administrativo
				inv.setDescription(hosp.get_ValueAsString("text"));
				inv.save();
				
				if(inv != null)
					salida = salida+"Se ha generado la factura "+inv.getDocumentNo();
				
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				
				String sqlgetlabdetail = "SELECT med_labdetail_id "
						+ " FROM med_labdetail m "
						+ " LEFT JOIN m_product mp ON (m.M_product_ID = mp.m_product_id)"
						+ " WHERE cc_hospitalization_id = "+p_Hospitalization_ID
						+ " AND (mp.IsTaxed = 'Y' OR mp.IsTaxed IS NULL)"
						+ " AND m.MED_PhysicalBond <> 'Y'"
						+ " AND InsurerLine <= 0";
				...
				{	
					...
					priceactual = lab.getAmount().divide(lab.getQty(),RoundingMode.HALF_UP).divide(new BigDecimal(1.19),RoundingMode.HALF_UP);
					
					
					invl.setPriceActual(priceactual.setScale(0,RoundingMode.HALF_UP));
					
					BigDecimal priceentered = Env.ZERO;
					priceentered = lab.getAmount().divide(lab.getQty(),RoundingMode.HALF_UP).divide(new BigDecimal(1.19), RoundingMode.HALF_UP);
					invl.setPriceEntered(priceentered.setScale(0,RoundingMode.HALF_UP));
					invl.setQtyInvoiced(lab.getQty());
					invl.setLineNetAmt(lab.getAmount());
					invl.setC_Tax_ID(2000002);
					//invl.setC_Tax_ID(2000003);
					invl.save();
				}
				hosp.set_CustomColumn("C_Invoice_ID", inv.get_ID());
				hosp.save();
				//se completa factura //no se completara factura ni generara xml para ingreso ambulatorio
				/*inv.processIt("CO");
			...
			//se intenta crear boleta exenta
			int cantExenta = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1)"
					+ " FROM med_labdetail m "
					+ " INNER JOIN m_product mp ON (m.M_product_ID = mp.m_product_id)"
					+ " WHERE cc_hospitalization_id = "+p_Hospitalization_ID
					+ " AND mp.IsTaxed <> 'Y' AND m.MED_PhysicalBond <> 'Y'"
					+ " AND InsurerLine <= 0");
			if(cantExenta > 0)
			//se crea boleta exenta				
			{
				MInvoice invEx = new MInvoice(getCtx(), 0, get_TrxName());
				invEx.setAD_Org_ID(hosp.getAD_Org_ID());
				invEx.setC_DocTypeTarget_ID(2000172);
				invEx.setC_BPartner_ID(hosp.getC_BPartner_ID());
				invEx.setDateInvoiced(hosp.getUpdated());
				invEx.setDateAcct(hosp.getUpdated());
				//se copia campo desde ingreso administrativo
				invEx.setDescription(hosp.get_ValueAsString("text"));
				invEx.save();
				
				if(invEx != null)
					salida = salida+" - Se ha generado la factura exenta "+invEx.getDocumentNo();
				
				
				PreparedStatement pstmtEx = null;
				ResultSet rsEx = null;
				
				String sqlgetlabdetailEx = "SELECT med_labdetail_id "
						+ "FROM med_labdetail m"
						+ " INNER JOIN m_product mp ON (m.M_product_ID = mp.m_product_id)"
						+ " WHERE cc_hospitalization_id = "+p_Hospitalization_ID
						+ " AND mp.IsTaxed = 'N'"
				...
				pstmtEx = DB.prepareStatement(sqlgetlabdetailEx, get_TrxName());
				rsEx = pstmtEx.executeQuery();
				X_MED_LabDetail labEx = null;

				while(rsEx.next())
				{
					
					labEx = new X_MED_LabDetail(getCtx(), rsEx.getInt(1), get_TrxName());
					if(labEx.getQty().compareTo(Env.ZERO) <= 0)
						continue;
					MInvoiceLine invl = new MInvoiceLine(getCtx(), 0, get_TrxName());
					invl.setC_Invoice_ID(invEx.get_ID());
					invl.setAD_Org_ID(invEx.getAD_Org_ID());
					if(labEx.getM_Product_ID() > 0)
						invl.setM_Product_ID(labEx.getM_Product_ID());
					else
						invl.setC_Charge_ID(2000000);
					invl.setQtyEntered(labEx.getQty());
					invl.setPriceActual(labEx.getAmount().divide(labEx.getQty()));
					invl.setPriceEntered(labEx.getAmount().divide(labEx.getQty()));
					invl.setQtyInvoiced(labEx.getQty());
					invl.setLineNetAmt(labEx.getAmount());
					invl.setC_Tax_ID(2000003);					
					invl.save();
				}
				hosp.set_CustomColumn("C_InvoiceRef_ID", invEx.get_ID());
				hosp.save();
				//se completa factura ////no se completara factura ni generara xml para ingreso ambulatorio 
				/*invEx.processIt("CO");
				invEx.saveEx(get_TrxName());
				//se crea xml 
				if(invEx.getC_DocType().getName().toLowerCase().contains("boleta"))
					msg=msg+"-"+CreateXMLCGBoleta(invEx);
				else
					msg=msg+"-"+CreateXMLCG(invEx);*/	
	...
	public String CreateXMLCGBoleta(MInvoice invoice)
    {
        ...
        try
        {
          ...
          //ininoles calculo de fecha de vencimiento
            ...
            //if(!invoice.getC_DocType().getName().toLowerCase().contains("exenta"))
            //{   
            Element IVA = document.createElement("IVA");
            BigDecimal ivaamt= Env.ZERO;
            if(amountex.intValue()!=invoice.getGrandTotal().intValue())
            ...
            mylog = "archivo";
	          ...
	          Transformer transformer = TransformerFactory.newInstance().newTransformer();
	          transformer.setOutputProperty("indent", "yes");
	          //transformer.setOutputProperty("encoding", "UTF-8");ISO-8859-1
	          transformer.setOutputProperty("encoding", "UTF-8");
	          transformer.transform(source, result);
	          transformer.transform(source, console);
