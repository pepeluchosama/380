...
/**
 *	Validator for CC
 *  Update Ambulatory labs amount
 *  @author mfrojas
 */
public class ModCCUpdateMedLab implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModCCUpdateMedLab ()
	{
		super ();
	}	//	MyValidator
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if((type == TYPE_BEFORE_NEW || type == TYPE_BEFORE_CHANGE ) && po.get_Table_ID()==X_MED_LabDetail.Table_ID) 
		{	
			...
			{	
				...
				int version = DB.getSQLValue(po.get_TrxName(), "SELECT coalesce(max(m_pricelist_version_id),0) from m_pricelist_version" +
					" WHERE isactive='Y' AND m_pricelist_id = "+pricelist);
				BigDecimal amountproduct = Env.ZERO;
				if(version > 0)
				{
					int prod = lab.getM_Product_ID();
					amountproduct = DB.getSQLValueBD(po.get_TrxName(), "SELECT coalesce(max(pricelist),0) from m_productprice" +
