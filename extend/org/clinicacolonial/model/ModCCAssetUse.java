 ...
 *  @author mfrojas
 */
public class ModCCAssetUse implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModCCAssetUse ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModCCAssetUse.class);
	/** Client			*/
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		{	
			...
			String sqlupdate2 = "Update a_Asset set c_bpartner_id = "+actualpartner+" where a_asset_id = "+actualasset;
			DB.executeUpdate(sqlupdate2,po.get_TrxName());

		}
		
		if(type == TYPE_BEFORE_CHANGE && po.get_Table_ID()==X_A_Asset_Use.Table_ID) 
		{	
			X_A_Asset_Use useregister = (X_A_Asset_Use)po;
			int actualpartner = useregister.get_ValueAsInt("c_bpartner_id");
			log.config("usuario actual "+actualpartner);
