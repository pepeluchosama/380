 ...
 */
public class ProcessSSomac extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		if (p_SSomac > 0)
		{
			X_OFB_Ssomac somac = new X_OFB_Ssomac(getCtx(),p_SSomac, get_TrxName());
			...
			int cant = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1) FROM AD_Document_Action_Access daa" +
					...
					" AND C_DocType_ID = "+somac.getC_DocType_ID());
			//procesar
			if(cant > 0)
			{
				if(newStatus.compareTo("CO")==0 || newStatus.compareTo("VO")==0)
				{
					somac.setDocStatus(newStatus);							
					somac.setProcessed(true);
				}
				else
					somac.setDocStatus(newStatus);						
				somac.save();
