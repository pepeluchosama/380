 ...
 *  @version $Id: ProcessSSomac.java $
 */
public class ProcessDriversControl extends SvrProcess
{
	private String			p_DocStatus = null;
	private int				p_SSomac = 0; 
	/**
	...
	protected String doIt() throws Exception
	{
		if (p_SSomac > 0)
		{
			X_TP_DriversControl driver = new X_TP_DriversControl(getCtx(),p_SSomac, get_TrxName());
			...
			int cant = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1) FROM AD_Document_Action_Access daa" +
					...
					" AND C_DocType_ID = "+driver.getC_DocType_ID());
			//procesar
			if(cant > 0)
			{
				if(newStatus.compareTo("CO")==0 || newStatus.compareTo("VO")==0)
				{
					driver.setDocStatus(newStatus);							
					driver.setProcessed(true);
				}
				else
					driver.setDocStatus(newStatus);						
				driver.save();
