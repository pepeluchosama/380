 ...
 */
public class ImportInvoiceTSM extends SvrProcess
{
	...
	//ininoles fecha de la importacion
	private Timestamp		m_DateInvoiced = null;
	//flota de la importacion
	private int				m_C_ProjectOFB_ID = 0;
...
//		A_Asset_ID2 
		...
		//ininoles actualizamos activo  
			sql = new StringBuffer ("UPDATE I_Invoice o "
					  + "SET A_Asset_ID=(SELECT MAX(A_Asset_ID) FROM A_Asset bp"
					  + " WHERE o.AssetValue=bp.Value AND o.AD_Client_ID=bp.AD_Client_ID) "
		...
		try
		{
			...
			while (rs.next ())
			{
					...
					|| !oldDocumentNo.equals(cmpDocumentNo)	)
				{
					...
					//
					//ininoles fecha como parametro
					if (m_DateInvoiced != null)
					{
						invoice.setDateInvoiced(m_DateInvoiced);
						invoice.setDateAcct(m_DateInvoiced);
					}
					if (m_C_ProjectOFB_ID > 0)
					{
						invoice.set_CustomColumn("C_ProjectOFB_ID", m_C_ProjectOFB_ID);						
					}
					//ininoles campos nuevo TP_Transport
					
					String sqlTP = "select min(tp_transport) FROM I_Invoice WHERE I_IsImported='N'";
					String tp_transport = DB.getSQLValueString(get_TrxName(), sqlTP);
					if (tp_transport != null)
					{
						//invoice.set_CustomColumn("TP_Transport",imp.get_ValueAsString("TP_Transport"));
						invoice.set_ValueOfColumn("TP_Transport", tp_transport);
					}
					//end ininoles
					invoice.save();
					noInsert++;
				...
				//ininoles campos para tsm
				line.set_ValueOfColumn("TP_DocumentNo", imp.get_Value("TP_DocumentNo"));
				line.set_ValueOfColumn("TP_Trip", imp.get_Value("TP_Trip"));
				line.set_ValueOfColumn("TP_A_Asset", imp.get_Value("TP_A_Asset"));
				line.set_ValueOfColumn("TP_Mod", imp.get_Value("TP_Mod"));
				line.set_ValueOfColumn("TP_Type", imp.get_Value("TP_Type"));
				line.set_ValueOfColumn("TP_Account", imp.get_Value("TP_Account"));
				line.set_ValueOfColumn("TP_Customer", imp.get_Value("TP_Customer"));
				line.set_ValueOfColumn("TP_Location", imp.get_Value("TP_Location"));				
				line.set_ValueOfColumn("TP_SumVol", imp.get_Value("TP_SumVol"));
				line.set_ValueOfColumn("TP_SumKM", imp.get_Value("TP_SumKM"));
				line.set_ValueOfColumn("TP_SumAmount", imp.get_Value("TP_SumAmount"));
				line.set_ValueOfColumn("TP_VNR", imp.get_Value("TP_VNR"));
				line.set_ValueOfColumn("TP_ProdB", imp.get_Value("TP_ProdB"));
				line.set_ValueOfColumn("TP_DateTrx", imp.get_Value("TP_DateTrx"));
				
				//ininoles nueva columna activo
				if(imp.get_ValueAsInt("A_Asset_ID") > 0)
				{
					line.set_CustomColumn("A_CreateAsset", true);
					line.set_CustomColumn("A_CapvsExp", "Cap");
					line.set_CustomColumn("A_Asset_ID", imp.get_ValueAsInt("A_Asset_ID"));
				}
				line.save();
				//
