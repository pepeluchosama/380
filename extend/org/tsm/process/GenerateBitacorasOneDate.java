 ...
 */
public class GenerateBitacorasOneDate extends SvrProcess
{	
	/** Warehouse				*/
	...
	{	
		if(p_DateTrxFrom == null)
		{
			p_DateTrxFrom = new Timestamp(System.currentTimeMillis());
			p_DateTrxFrom.setNanos(0);
			p_DateTrxFrom.setSeconds(0);
			p_DateTrxFrom.setMinutes(0);
			p_DateTrxFrom.setHours(0);
		}
		...
		String sqlHR = "SELECT pm.M_Movement_ID,pm.C_BPartner_ID, pm.AD_Org_ID, pm.TP_Asset_ID as A_Asset_ID, pm.movementdate " +
				...
				" AND pm.MovementDate = ? AND pm.DocStatus IN ('CO','CL','DR')";
		if(p_Org_ID > 0)
			...
			while (rsHR.next())
			{
				...
				if(ID_BPartner > 0)
				{
					...
					else//no existe bitacora
					{
						...
						bitacora.setDateTrx(p_DateTrxFrom);
