 ...
 */
public class GenerateBitacorasConPreHR extends SvrProcess
{	
	/** Warehouse				*/
	...
	{	
		...
		//agregamos pre hojas de ruta
		//Ingreso de prebitacoras 
		String sqlHR = "SELECT pml.Pre_M_MovementLine_ID,pml.C_BPartner_ID, pm.AD_Org_ID, pml.A_Asset_ID, pm.movementdate " +
				" FROM Pre_M_Movement pm " +
				" INNER JOIN Pre_M_MovementLine pml ON (pm.Pre_M_Movement_ID = pml.Pre_M_Movement_ID) " +
				" WHERE pml.Pre_M_MovementLine_ID NOT IN (SELECT Pre_M_MovementLine_ID FROM hr_bitacoraline biLine" +
				" INNER JOIN hr_bitacora bi ON (bi.HR_Bitacora_ID = biLine.HR_Bitacora_ID)WHERE DocStatus IN ('CO','DR','IP') " +
				" AND Pre_M_MovementLine_ID IS NOT NULL) AND pm.AD_Client_ID = "+Env.getAD_Client_ID(getCtx())+
				" AND pm.MovementDate BETWEEN ? AND ? ";
		if(p_Org_ID > 0)
			sqlHR = sqlHR + " AND pm.AD_Org_ID = "+p_Org_ID+" AND pm.DocStatus IN ('CO','CL')";
