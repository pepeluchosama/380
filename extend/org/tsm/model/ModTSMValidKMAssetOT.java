 ...
 */
public class ModTSMValidKMAssetOT implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModTSMValidKMAssetOT ()
	{
		super ();
	}	//	MyValidator
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if((type == TYPE_AFTER_NEW || type == TYPE_AFTER_CHANGE)&& po.get_Table_ID()==X_MP_OT.Table_ID && po.is_ValueChanged("DocStatus"))  
		{
			...
			if(ot.getDocStatus().compareTo("CO") == 0)
			{
				...
				{
					BigDecimal amtLog = DB.getSQLValueBD(po.get_TrxName(), "SELECT MAX(ml.amt) " +
							" FROM A_Asset aa " +
