 ...
 *  @author ininoles
 */
public class ModelTSMCriticalDate implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModelTSMCriticalDate ()
	{
		super ();
	}	//	MyValidator
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if((type == TYPE_AFTER_CHANGE || type == TYPE_AFTER_NEW) && po.get_Table_ID()==MBPartner.Table_ID)
		{
			MBPartner bPart = (MBPartner)po;
			if(bPart.get_ValueAsBoolean("IsUnLinked"))
			{
				DB.executeUpdate("UPDATE C_CriticalDate SET Status = 'BA' WHERE C_BPartner_ID="+bPart.get_ID(),po.get_TrxName());
			}			
		}
		if((type == TYPE_AFTER_CHANGE || type == TYPE_AFTER_NEW) && po.get_Table_ID()==MAsset.Table_ID)
		{
			MAsset asset = (MAsset)po;
			if(!asset.isActive())
			{
				DB.executeUpdate("UPDATE C_CriticalDate SET Status = 'BA' WHERE A_Asset_ID="+asset.get_ID(),po.get_TrxName());
			}			
		}
		return null;
	}	//	modelChange
	...
	public String docValidate (PO po, int timing)
	{
		...
		if(timing == TIMING_AFTER_COMPLETE && po.get_Table_ID()==MCash.Table_ID)
		{
			...
			begbalance = Env.ZERO;
			//sql que validará el saldo inicial
...
//				return "El diario con nro documento "+cash.getDocumentNo()+" tiene saldo inicial erróneo";
