...
/**
 *	Validator for client movix
 *
 *  @author Italo Niñoles
 */
public class ModelMovixOrder implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModelMovixOrder ()
	{
		super ();
	}	//	MyValidator
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if (type == TYPE_AFTER_NEW && po.get_Table_ID()==MOrderLine.Table_ID)
		{
			...
			if(oLine.get_ValueAsInt("M_RequisitionLine_ID")>0 && order.isSOTrx() == false)
			{
				...
				/*if(req.get_ValueAsInt("C_Bpartner_ID") > 0)
				{
					...
					order.setBill_BPartner_ID(req.get_ValueAsInt("C_Bpartner_ID"));
				}
				if(req.get_ValueAsInt("C_Bpartner_Location_ID") > 0)
				{
					order.setC_BPartner_Location_ID(req.get_ValueAsInt("C_Bpartner_Location_ID"));
					order.setBill_Location_ID(req.get_ValueAsInt("C_Bpartner_Location_ID"));					
				}*/
				if(req.get_ValueAsInt("C_Bpartner_ID") > 0)
				{
		...
		{
			MOrderLine oLine = (MOrderLine) po;
			if(oLine.getPriceEntered().compareTo(Env.ZERO) <=0)
