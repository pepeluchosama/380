...
public class CalloutOrder extends CalloutEngine
{
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		//no se validara estock en callout, sino en validacion de model
		/*if (Env.isSOTrx(ctx, WindowNo))
		{
			if (product.isStocked())
			{
				...
				//ininoles se sobreescribe disponible por funcion de BD
				available = DB.getSQLValueBD(null, "SELECT SUM(bomqtyavailable("+M_Product_ID.intValue()+",ml.M_Warehouse_ID,ml.M_Locator_ID))" +
								" FROM M_Locator ml WHERE M_Warehouse_ID = "+M_Warehouse_ID);
				if (available == null)
					available = Env.ZERO;
