 ...
 */
public class ModelArtExportDTE implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModelArtExportDTE ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModelArtExportDTE.class);
	/** Client			*/
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if ((type == TYPE_AFTER_NEW || type == TYPE_AFTER_CHANGE)&& po.get_Table_ID()==MInvoice.Table_ID)
		{
			...
			if(inv.isSOTrx() && inv.getDocStatus().compareTo("CO") == 0)
			{
				String ret = CreateXMLCG(inv);
				log.config(ret);
			}
		}
		return null;
