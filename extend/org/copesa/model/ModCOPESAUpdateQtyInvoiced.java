 ...
 */
public class ModCOPESAUpdateQtyInvoiced implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModCOPESAUpdateQtyInvoiced ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModCOPESAUpdateQtyInvoiced.class);
	/** Client			*/
	...
	public String docValidate (PO po, int timing)
	{
		...
		if(timing == TIMING_AFTER_COMPLETE && po.get_Table_ID()==MInvoice.Table_ID)
		{
			...
			if(inv.isSOTrx() && inv.getC_Order_ID() > 0)
			{
				...
				DB.executeUpdate("UPDATE C_OrderLine SET QtyInvoiced = 0 WHERE C_OrderLine_ID IN " +
						...
						" WHERE ol.C_Order_ID = "+order.get_ID()+" AND ol.IsActive = 'Y' AND upper(pc.value) like 'EDITO%')", po.get_TrxName());
			}
		}
		return null;
	}	//	docValidate
