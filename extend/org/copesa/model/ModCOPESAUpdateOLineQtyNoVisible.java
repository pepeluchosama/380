 ...
 */
public class ModCOPESAUpdateOLineQtyNoVisible implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModCOPESAUpdateOLineQtyNoVisible()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModCOPESAUpdateOLineQtyNoVisible.class);
	/** Client			*/
			...
			if (order.isSOTrx() && order.getDocStatus().compareToIgnoreCase("IP") == 0)
			{
				if (order.isSOTrx() && order.getDocStatus().compareToIgnoreCase("CO") != 0)
				{
					...
					for (int i = 0; i < lines.length; i++)
					{
						...
						if(!oLine.get_ValueAsBoolean("IsFree") && oLine.getM_Product_ID() > 0)
						{
							...
							if(cant == -1)
							{
								...
								order.setTotalLines(order.getTotalLines().subtract(oLine.getLineNetAmt()));								
