 ...
 */
public class ModCOPESAOrderSameLocator implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModCOPESAOrderSameLocator ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModCOPESAOrderSameLocator.class);
	/** Client			*/
	...
	public String docValidate (PO po, int timing)
	{
		...
		if(timing == TIMING_BEFORE_COMPLETE && po.get_Table_ID()==MOrder.Table_ID)
		{
			...
			if(order.isSOTrx())
			{
				...
				if(ISSameBPLocator)
				{
					int cant = DB.getSQLValue(po.get_TrxName(), "SELECT COUNT(DISTINCT(C_BPartner_Location_ID)) " +
							" FROM C_OrderLine WHERE C_Order_ID = "+order.get_ID());				
					if(cant > 1)
						return "Error: Direcciones no concordantes";
				}
			}			
		}
		return null;
