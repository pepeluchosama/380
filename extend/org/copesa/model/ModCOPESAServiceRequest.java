 ...
 */
public class ModCOPESAServiceRequest implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModCOPESAServiceRequest ()
	{
		super ();
	}	//	MyValidator
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if((type == TYPE_BEFORE_NEW || type == TYPE_BEFORE_CHANGE)&& po.get_Table_ID()==X_R_ServiceRequest.Table_ID) 
		{	
			...
			if( !validateFilter(req) )
				return "Campos para filtrar no calzan con ninguna línea del contrato/nota de venta";
			
			//seteo de nodo inicial
			int nodeid = req.getAD_WF_Node_ID();
			if(nodeid < 1 || type == TYPE_BEFORE_NEW)
			{
				nodeid = rType.get_ValueAsInt("AD_WF_Node_ID");
				if(nodeid > 0)
				{
	...
	private boolean validateFilter(X_R_ServiceRequest _req)
	{
		...
		if( _req.get_ValueAsInt("C_BPartner_ID") > 0  )
		    sql += " and C_BPartnerRef_ID = " + _req.get_ValueAsInt("C_BPartner_ID");

		if( _req.getC_OrderLine_ID() > 0  )
		    sql += " and C_OrderLine_ID = " + _req.getC_OrderLine_ID();

		if( _req.get_ValueAsInt("C_BPartner_Location_ID") > 0  )
		    sql += " and C_BPartner_Location_ID = " + _req.get_ValueAsInt("C_BPartner_Location_ID");
		
		sql += " fetch first row only";
		
	    int result = DB.getSQLValue(_req.get_TrxName(), sql);
