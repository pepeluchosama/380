 ...
 */
public class ModCOPESAValidMAmtLine implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModCOPESAValidMAmtLine ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModCOPESAValidMAmtLine.class);
	/** Client			*/
		...
		if(timing == TIMING_BEFORE_COMPLETE && po.get_Table_ID()==MOrder.Table_ID)
		{
			...
			if(order.isSOTrx())
			{
				if(order.getPaymentRule().compareTo("D") == 0)
				{
					...
					if(cant > 0)
						return "Error: No Existen Precios PAT. Selecione otra Regla de Pago ";
				}
			}			
		}
		return null;
