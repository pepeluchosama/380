 ...
 *	
 */
public class GenerateOTFromROrder extends SvrProcess
{	
	/**
	...
	{	
		...
		try
		{
			...
			while (rs.next())
			{
				...
				ta.saveEx();				
				createOTResourceDetail(ID_OrderLine,rs.getInt("MP_JobStandar_Task_ID"), ta.get_ID());
			}
			rs.close();
			pstmt.close();
			pstmt = null;
	...
	public boolean createOTResourceDetail(int ID_OrderLine, int ID_JobTask, int ID_NewTask)
	{	
		//creación de recursos OT con tarea
		String sql = "select * from MP_OT_ResourceOrder where C_OrderLine_ID = ? AND MP_OT_Task_ID = "+ID_JobTask;
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement(sql,null);
			pstmt.setInt(1,ID_OrderLine);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next())
			{
				...
				//if(ID_NewTask > 0)
					re.setMP_OT_Task_ID(ID_NewTask);
				/*else
				{
					int ID_OTTaskR = DB.getSQLValue(get_TrxName(), "SELECT MAX(MP_OT_Task_ID) FROM MP_OT_ResourceOrder where C_OrderLine_ID = "+ID_OrderLine);
					if(ID_OTTaskR > 0)
						re.setMP_OT_Task_ID(ID_OTTaskR);
				}*/
		...
		//recursos sin tarea
		/*String sql = "select * from MP_OT_ResourceOrder where C_OrderLine_ID = ? AND MP_OT_Task_ID IS NULL ";
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement(sql,null);
