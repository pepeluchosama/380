 ...
 */
public class ModWindsorValidOLine implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModWindsorValidOLine ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModWindsorValidOLine.class);
	/** Client			*/
		...
		if((type == TYPE_BEFORE_NEW || type == TYPE_BEFORE_CHANGE) && po.get_Table_ID()== MOrderLine.Table_ID)  
		{
					...
					&& order.getC_DocTypeTarget().getDocSubTypeSO().compareTo("SO") == 0) 
			{
				if(oLine.getM_Product_ID() > 0)
				{
					...
					{
						cant = DB.getSQLValue(po.get_TrxName(), "SELECT COUNT(*) FROM C_Orderline" +
							" WHERE C_Order_ID = "+order.get_ID()+" AND M_Product_ID = "+oLine.getM_Product_ID()+
					...
					else
					{
						cant = DB.getSQLValue(po.get_TrxName(), "SELECT COUNT(*) FROM C_Orderline" +
								" WHERE C_Order_ID = "+order.get_ID()+" AND M_Product_ID = "+oLine.getM_Product_ID()+
								" AND C_OrderLine_ID <> "+oLine.get_ID()+
								" AND (M_RequisitionLine_ID IS NULL OR M_RequisitionLine_ID = 0)");
					}
					if(cant > 0)
						return "ERROR: No se pueden ingresar lineas repetidas";
				}
			}
			if(order.isSOTrx() && (order.getDocStatus().compareTo("CO") != 0 && order.getDocStatus().compareTo("IP") != 0)
					&& order.getC_DocTypeTarget().getDocSubTypeSO().compareTo("SO") == 0) 
				...
				if(oLine.getM_Product_ID() > 0)
				{
					if(oLine.get_ValueAsInt("M_RequisitionLine_ID") > 0)
					{
						...
						if(oLine.getM_Product_ID() != rLine.getM_Product_ID())
							return "ERROR: Producto de solicitud no corresponde";
					}
				}
			}
		}
