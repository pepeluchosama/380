...
public class CalloutInventory extends CalloutEngine
{
	...
	public String qty (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		if (isCalloutActive())
			return "";
		MInventory inv = new MInventory(ctx, (Integer)mTab.getValue("M_Inventory_ID"),null);
		MInventoryLine invL = new MInventoryLine(ctx, (Integer)mTab.getValue("M_InventoryLine_ID"),null);
		BigDecimal qtyCount = (BigDecimal)mTab.getValue("QtyCount");
		if(qtyCount == null)
			qtyCount = Env.ZERO;
		Integer M_Product_ID = (Integer)mTab.getValue("M_Product_ID");		
		//se le suma la cantidad de todas las lineas
		BigDecimal qtyCountTotal = DB.getSQLValueBD(null, "SELECT SUM(QtyCount) FROM M_InventoryLine " +
				" WHERE M_Product_ID = "+M_Product_ID+" AND M_Inventory_ID ="+inv.get_ID()+" AND M_InventoryLine_ID <> "+invL.get_ID());
		if(qtyCountTotal == null)
			qtyCountTotal = Env.ZERO;
		qtyCount = qtyCount.add(qtyCountTotal);
		//disponible
		BigDecimal qtyAvai = Env.ZERO; 		
		if(inv.getAD_Client_ID() == 1000000)
		{
			qtyAvai= DB.getSQLValueBD(null, "SELECT qtyavailableofb(mp.M_Product_ID,"+inv.getM_Inventory_ID()+") FROM M_Product mp WHERE mp.M_Product_ID = "+M_Product_ID);
			if(qtyAvai == null)
				qtyAvai = Env.ZERO;
			BigDecimal aux = DB.getSQLValueBD(null, "SELECT qtyavailableofb(mp.M_Product_ID,1000010) FROM M_Product mp WHERE mp.M_Product_ID = "+M_Product_ID);
			if(aux == null)
				aux = Env.ZERO;
		...
		{
			qtyAvai= DB.getSQLValueBD(null, "SELECT bomqtyavailable(mp.M_Product_ID,"+inv.getM_Warehouse_ID()+",0) FROM M_Product mp WHERE mp.M_Product_ID = "+M_Product_ID);
		}
		if(qtyAvai == null)
			qtyAvai = Env.ZERO;
		
		//cantidad existente		
		BigDecimal qtyOnHand = DB.getSQLValueBD(null, " SELECT nvl(sum(s.qtyonhand),0) from m_storage s " +
				" inner join m_locator l on (s.m_locator_id=l.m_locator_id) " +
				" where s.M_Product_ID="+M_Product_ID+" and l.m_warehouse_id=" +inv.getM_Warehouse_ID()+
				" and l.VALUE not like '%MM%'");
		if(qtyOnHand == null)
			qtyOnHand = Env.ZERO;
		//diferencia
		BigDecimal dif = qtyOnHand.subtract(qtyAvai);
		
		if(qtyCount.compareTo(qtyOnHand) < 0)
		{
			if(dif.compareTo(qtyCount) > 0)
				return "ERROR: Se Generara un quiere de stock al bajar de "+qtyOnHand+" a "+qtyCount;			
		}
		return "";
	}   //  product
